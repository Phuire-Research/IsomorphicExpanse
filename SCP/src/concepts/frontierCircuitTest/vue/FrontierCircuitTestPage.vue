...
import { FRONTIERCIRCUITTEST_TABS, type FrontierCircuitTestDesignation, type FrontierCircuitTestTab } from '../frontierCircuitTest.type';
// Cycle 159 D1 · IUPA · frontierCircuitTest muxified per-page (no longer base)
...
const designations = ref<FrontierCircuitTestDesignation[]>([]);
const activeDesignationName = ref<string>('');
const activeTab = ref<FrontierCircuitTestTab>('info');
...
const activeDesignation = computed<FrontierCircuitTestDesignation | null>(() => {
...
function switchTab(tab: FrontierCircuitTestTab) {
  if (!muxium) return;
  muxium.dispatch(
    (muxium as Muxium<ClientMuxiumDeck>).deck.d.client.d.frontierCircuitTest.e.frontierCircuitTestSetActiveTab({ tab }),
  );
}
...
onMounted(() => {
  ...
  // Cycle 159 D1 · IUPA · landing supplies frontierCircuitTest as muxonomic page concept
  ...
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'frontierCircuitTestPageSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            designations.value = d.client.d.frontierCircuitTest.k.designations.select();
            activeDesignationName.value = d.client.d.frontierCircuitTest.k.activeDesignationName.select();
            activeTab.value = d.client.d.frontierCircuitTest.k.activeTab.select();
            loadedDiamondContent.value = d.client.d.frontierCircuitTest.k.loadedDiamondContent.select();
            loadedOnyxContent.value = d.client.d.frontierCircuitTest.k.loadedOnyxContent.select();
            loadedBoundCascade.value = d.client.d.frontierCircuitTest.k.loadedBoundCascade.select();
            loadedFileSystemSheet.value = d.client.d.frontierCircuitTest.k.loadedFileSystemSheet.select();
          },
          {
            selectors: [
              d__.client.d.frontierCircuitTest.k.designations,
              d__.client.d.frontierCircuitTest.k.activeDesignationName,
              d__.client.d.frontierCircuitTest.k.activeTab,
              d__.client.d.frontierCircuitTest.k.loadedDiamondContent,
              d__.client.d.frontierCircuitTest.k.loadedOnyxContent,
              d__.client.d.frontierCircuitTest.k.loadedBoundCascade,
              d__.client.d.frontierCircuitTest.k.loadedFileSystemSheet,
            ],
          },
        ),
      ]),
...
<template>
  <div class="frontierCircuitTest-page">
    ...
    <nav class="tab-nav">
      <button
        v-for="tab in FRONTIERCIRCUITTEST_TABS"
        :key="tab.value"
    ...
    <main class="tab-content">
      <section v-if="activeTab === 'info'" class="info-tab">
        ...
        <p v-else class="placeholder">
          (info-sheet not loaded — register sample designations from FrontierCircuitTestLanding for visual preview)
        </p>
      </section>
      ...
      <section v-else-if="activeTab === 'doviewer'" class="doviewer-tab">
        ...
        <div class="doviewer-grid">
          <div class="doviewer-pane">
            ...
            <p v-else class="placeholder">
              (Diamond not loaded — register sample designations from FrontierCircuitTestLanding)
            </p>
          </div>
          <div class="doviewer-pane">
            <h3>Onyx (Lambda)</h3>
            ...
            <p v-else class="placeholder">
              (Onyx not loaded — register sample designations from FrontierCircuitTestLanding)
            </p>
          </div>
        </div>
      </section>
