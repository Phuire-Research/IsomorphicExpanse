 ...
 * the frontierCircuitTest CLIENT face per-page; createFrontierCircuitTestClientConcept() returns the SCBM
 * muxifyConcepts node — frontierCircuitTest (Tier 1) carrying the ONE shared suiteCascade member
 * (Tier 2 · d.suiteCascade). The MPRF registration Principle seeds the
 * `frontierCircuitTests` roster at boot.
 *
 ...
 *   Home          -> the Suite 8 roster (frontierCircuitTests · Tier 1) + the docked-cascade
 *                    context (cascades · Tier 2 · the SCBM payoff)
 *   Component     -> CPLD live render — ODSS spawn (A-4) + PFGD list (A-5), keyed
 *                    by the frontierCircuitTests plurality
 *   Documentation -> the FrontierCircuitTest registration/assignment reference (static markdown)
 *
 * MIGRATION (deferred A-1 item): the Home roster reads `frontierCircuitTests` (the SPSR Record)
 * via DECK K, NOT the legacy `designations[]` array. The legacy array + its 3
 * sync'd qualities remain on the concept (FrontierCircuitTestPage.vue still reads them) — this
 * Landing's roster is the canonical `frontierCircuitTests` surface.
 *
 * Citation: SuiteCascadeLanding.vue (HCD triad v-if switch · DIRECT bearing · B-6).
 * Citation: FrontierCircuitTestOnDemand.vue / FrontierCircuitTestSessionList.vue (IUPA + GPIM controller idiom).
 * Citation: MASTER-DIAMOND-FRONTIERCIRCUITTEST-CONCEPT-ASPIRANT.md §3 (HCD) + §5 (Diametric Conference).
 */
...
import type { FrontierCircuitTestEntry, FrontierCircuitTestSubPage } from '../frontierCircuitTest.type';
...
// A-1 SCBM · IUPA · createFrontierCircuitTestClientConcept() returns the muxifyConcepts node —
// frontierCircuitTest (Tier 1) carrying the ONE shared suiteCascade member (Tier 2).
...
import { FRONTIERCIRCUITTEST_SUB_PAGE_OPTIONS } from '../frontierCircuitTest.subPageRegistry';
...
import FrontierCircuitTestSubPageNav from './components/FrontierCircuitTestSubPageNav.vue';
import FrontierCircuitTestComponentSubPage from './components/FrontierCircuitTestComponentSubPage.vue';
import FrontierCircuitTestDocumentationSubPage from './components/FrontierCircuitTestDocumentationSubPage.vue';
...
import FrontierCircuitTestBiplane from './components/FrontierCircuitTestBiplane.vue';

// --- Reactive refs mirrored from frontierCircuitTest state via the stage-planner subscription.
// frontierCircuitTests + activeSubPage = Tier 1 (own); cascades + activeCascadeDirectory = Tier 2
// (the shared suiteCascade member · the SCBM payoff).
const frontierCircuitTests = ref<Record<string, FrontierCircuitTestEntry>>({});
const activeSubPage = ref<FrontierCircuitTestSubPage>('home');
...
const subPageOptions = FRONTIERCIRCUITTEST_SUB_PAGE_OPTIONS;

// ============================================
// MD-3 · THE NAME-FIRST DEMOMETRIC MINT — the Create UI + the sovereignty-pure roster
// ============================================
// SOVEREIGNTY: the roster reads THIS SCP's OWN Cascades/8_SUITES via GET /frontierCircuitTest/local-roster
// (SCP-local · no bridge SEAP dependency · no static-seed GAP). A minted Suite 8 appears the
// instant its dir exists — refreshLocalRoster re-fetches + dispatches frontierCircuitTestRegisterFrontierCircuitTest per
// entry into the SPSR Record (the existing subscription mirrors it into frontierCircuitTests.value).

// MD-5 · the local-roster entry now carries the SNIPPET (the first meaningful Instance.md line ·
// the extended /frontierCircuitTest/local-roster · the MD-4 firstLine helper idiom).
...
// MD-5 · snippet map keyed by Suite 8 name — held here (NOT on the FrontierCircuitTestEntry state · the
...
// D-EF-3 · THE FORGE PREDICATE map — keyed by Suite 8 name (held OFF the FrontierCircuitTestEntry state · same
...
// query param on mount; once the roster load resolves that entry into frontierCircuitTests, auto-expand it (the
...
// `/?island=frontierCircuitTest&s8=<name>`. The SAME auto-expand mechanism as ?minted=, but it opens the
...
// frontierCircuitTestRegisterFrontierCircuitTest per dir entry (idempotent · the reducer overwrites by name). Missing
// description/color (the local listing carries only name+path) → deterministic placeholders.
async function refreshLocalRoster() {
  if (!muxium) return;
  try {
    const r = await fetch('/s8/local-roster'); // C370-B · rename-proof prefix (aliased to /frontierCircuitTest)
    if (!r.ok) return;
    ...
    for (const e of entries) {
      ...
      const entry: FrontierCircuitTestEntry = {
        name: e.name,
      ...
      (muxium as Muxium<ClientMuxiumDeck>).dispatch(
        (muxium as Muxium<ClientMuxiumDeck>).deck.d.client.d.frontierCircuitTest.e.frontierCircuitTestRegisterFrontierCircuitTest({
          name: e.name,
          entry,
        }),
      );
    ...
    // that name (the present case is handled by the frontierCircuitTests watcher above).
...
async function submitCreate() {
  ...
  createError.value = '';
  try {
    const r = await fetch('/s8/create', { // C370-B · rename-proof prefix (aliased to /frontierCircuitTest)
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
// --- Home derivation · the Suite 8 roster (frontierCircuitTests Record · Tier 1).
const frontierCircuitTestEntries = computed<FrontierCircuitTestEntry[]>(() => Object.values(frontierCircuitTests.value));

// MD-5 · resolve the expanded selection to its entry (null → the compact grid).
const expandedEntry = computed<FrontierCircuitTestEntry | null>(() =>
  expandedName.value ? frontierCircuitTests.value[expandedName.value] ?? null : null,
);
...
function selectSubPage(subPage: FrontierCircuitTestSubPage) {
  subPageLog('user-click dispatch', subPage); // C834 instrumentation
  if (!muxium) return;
  muxium.dispatch(
    (muxium as Muxium<ClientMuxiumDeck>).deck.d.client.d.frontierCircuitTest.e.frontierCircuitTestSetActiveSubPage({
...
onMounted(() => {
  ...
  // frontierCircuitTests watcher above can auto-expand that entry the instant refreshLocalRoster resolves it.
  try {
  ...
  // IUPA · landing supplies frontierCircuitTest (SCBM node) as the muxonomic page concept.
  muxium = createClientMuxiumInstance<ClientMuxiumDeck>(
    ...
    {
      title: 'FrontierCircuitTestLanding',
      logging: true,
      storeDialog: true,
    },
  );
  ...
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'frontierCircuitTestLandingSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            // Tier 1 — own roster Record + local SubPage selector.
            frontierCircuitTests.value = d.client.d.frontierCircuitTest.k.frontierCircuitTests.select();
            ...
            // selection holds.
            const nextSubPage = d.client.d.frontierCircuitTest.k.activeSubPage.select();
            const inUnion = FRONTIERCIRCUITTEST_SUB_PAGE_OPTIONS.some((o) => o.value === nextSubPage);
            if (!inUnion) {
          ...
          {
            selectors: [
              d__.client.d.frontierCircuitTest.k.frontierCircuitTests,
              d__.client.d.frontierCircuitTest.k.activeSubPage,
  ...
  // dispatched frontierCircuitTestRegisterFrontierCircuitTest actions land on the bound base.
...
<template>
  <div class="frontierCircuitTest-landing">
    ...
    <main class="landing-content">
      <FrontierCircuitTestSubPageNav
        :options="subPageOptions"
        :active-sub-page="activeSubPage"
        @sub-page-selected="selectSubPage"
      />

      <!-- HOME · the Suite 8 roster (frontierCircuitTests · Tier 1) + docked-cascade context (Tier 2) -->
      <section v-if="activeSubPage === 'home'" class="home-subpage">
        ...
        <!-- The Suite 8 roster · frontierCircuitTests Record (Tier 1 · the migrated A-1 surface) -->
        <section class="roster-panel hifi-pane-amethyst">
          <h2 class="hifi-heading">Registered Suite 8 Roster</h2>

          <p v-if="frontierCircuitTestEntries.length === 0" class="roster-empty-note">
          ...
          <!-- MD-5 · THE ROSTER SURFACE SWAP — one FrontierCircuitTestCard (compact) per entry; card click -->
          ...
          <div v-else class="roster-grid">
            <FrontierCircuitTestCard
              v-for="entry in frontierCircuitTestEntries"
              :key="entry.name"
              :entry="entry"
            ...
            <div class="roster-create-cell">
              ...
              <div v-else class="create-form">
                <label class="create-field-label" for="frontierCircuitTest-create-name">
                  New Suite 8 Name
                </label>
                <input
                  id="frontierCircuitTest-create-name"
                  v-model="createName"
                  class="create-input"
                  type="text"
      ...
      <!-- COMPONENT · CPLD live render — ODSS spawn (A-4) + PFGD list (A-5) keyed by frontierCircuitTests -->
      <FrontierCircuitTestComponentSubPage
        v-else-if="activeSubPage === 'component'"
        :frontierCircuitTests="frontierCircuitTests"
      />

      <!-- DOCUMENTATION · the FrontierCircuitTest registration/assignment reference -->
      <FrontierCircuitTestDocumentationSubPage v-else-if="activeSubPage === 'documentation'" />
    </main>
  </div>
</template>

<style scoped>
.frontierCircuitTest-landing {
  min-height: 100vh;
  padding: 2rem;
