...
/**
 * FrontierCircuitTestHomeLanding — Template Suite 8 Page (DSSLS · Domain-Scoped SCP Landing Scaffold)
 *
 ...
 *   Zone 2 · SSMC                — ScsBridgeSessionManagement (mode=specific · frontierCircuitTestName)
 ...
 * name + tagline, binds the SSMC frontierCircuitTestName, and fills the work surface. Then it flips
...
// IUPA · the frontierCircuitTest CLIENT concept supplies this landing's page Muxium (mirrors CadmiumLanding
...
// SSMC · the importable Session Management component (mode=specific · frontierCircuitTestName) — the SAME
...
// IE-D4 · CREATE-S8 DYNAMIC LINKAGE · the LIVE D-O widget (the generic FrontierCircuitTestCascadeDocs · reads THIS
...
import FrontierCircuitTestCascadeDocs from './components/FrontierCircuitTestCascadeDocs.vue';
// PRE-EPOCH · decision 5 · the MINIMAL default standing menu (spawn-anchor row + documentation row).
import { FRONTIERCIRCUITTEST_DEFAULT_MENU_STAGE } from '../model/frontierCircuitTestDefaultMenu.model';
// W3 · THE ONE-BAR SUBNAV (D-EF-0) · the Card subpage mounts the FrontierCircuitTestCard by this page's display name.
import type { FrontierCircuitTestEntry } from '../frontierCircuitTest.type';
import FrontierCircuitTestCard from './components/FrontierCircuitTestCard.vue';
// W2 · THE FORGE MENU DOOR — the branch-aware two-state door reused VERBATIM from FrontierCircuitTestBiplane
...
// `frontierCircuitTestName` MUST byte-match the Suite 8 directory name (Cascades/8_SUITES/{name}/) so the
// SSMC mode=specific filter narrows the session list to THIS domain's sessions.
// ============================================================
// ADAPT: replace 'FrontierCircuitTest' with the user's Suite 8 designation (from Instance.md Designation).
const domainName = ref<string>('FrontierCircuitTest');
...
const frontierCircuitTestName = ref<string>('FrontierCircuitTest');
...
// designation. The N-watcher SMRP relay flows shatteriteMenus[frontierCircuitTestName] into here; absent a live
// stage it stays EMPTY_MENU_STAGE and ShatteriteMenu renders the FRONTIERCIRCUITTEST_DEFAULT_MENU_STAGE
...
// frontierCircuitTest state · populated by D4 Diametric reads · empty at first render → the pane shows its built-in
...
// widget + the Forge menu + the Shatterite Menu + the Session Manager); Card = the FrontierCircuitTestCard
// mounted by THIS page's display name (the biplane-tab idiom · FrontierCircuitTestBiplane's HOME|CARD standard).
// ============================================================
type HomeTab = 'home' | 'card';
const activeTab = ref<HomeTab>('home');

// W3 · THE CARD ENTRY — a minimal FrontierCircuitTestEntry keyed on THIS page's display name (frontierCircuitTestName). The
// local-roster fetch below fills its snippet + color; before it resolves the Card renders the name
// with the base placeholder (never a broken surface). FrontierCircuitTestCard consumes {entry, domain, snippet}.
const selfSnippet = ref<string>('');
const cardEntry = computed<FrontierCircuitTestEntry>(() => ({
  name: frontierCircuitTestName.value,
  directoryPath: `Cascades/8_SUITES/${frontierCircuitTestName.value}`,
  description: selfSnippet.value.length > 0 ? selfSnippet.value : 'Suite 8',
  color: '#9aa0a8',
}));

// ============================================================
// W2 · THE FORGE PREDICATE — the generated page knows its own 8_SUITES dir name (frontierCircuitTestName · the
// display-name binding). Fetch THIS SCP's local-roster and read the entry matching frontierCircuitTestName for
// its isUnactualized flag (the FrontierCircuitTestLanding idiom · the raw Instance.md still carries the scaffold's
...
// was minted by `frontierCircuitTest:page` but NO Cascades/8_SUITES/{display-name}/ scaffold exists yet, so THIS
...
// `frontierCircuitTest`/`FrontierCircuitTest` token, so `frontierCircuitTest:page`'s copy-move-rename can NEVER rewrite it) — a GENERATED
// page now reads a WORKING roster keyed by its DISPLAY name (frontierCircuitTestName · the 8_SUITES dir). The
...
async function refreshSelfForgeState(): Promise<void> {
  try {
    ...
    const self = entries.find((e) => e.name === frontierCircuitTestName.value);
    if (self) {
...
// ============================================================
// W2 · THE FORGE'S DOOR — Engage Entourage Forge (ANCHOR spawn · reused from FrontierCircuitTestBiplane VERBATIM).
...
async function engageEntourageForge(): Promise<void> {
  ...
  try {
    ...
    // ANCHOR spawn — the forge-door idiom (FrontierCircuitTestBiplane.engageEntourageForge). The Forge researches
    // + builds this domain, writing the real Domain into the minted Instance.md. C373 · triggerSpawnS8Session
    // (the rename-proof alias) — the `frontierCircuitTest:page` domain-token rewrite leaves `s8` intact against the
...
// mortal. resolvedScpName is filled on mount (best-effort · null → match by frontierCircuitTestName alone).
// ============================================================
...
// ============================================================
// W2 · THE TWO-STATE DOOR — branch-aware (reused from FrontierCircuitTestBiplane VERBATIM). Forging modifies this
...
onMounted(() => {
  ...
  console.log('[FrontierCircuitTestHomeLanding] mounted · build=C376-pointerfix-clickprobe');

  // IUPA · this landing supplies frontierCircuitTest as the muxonomic page concept (the SSMC controller
  ...
  muxium = createClientMuxiumInstance<ClientMuxiumDeck>(
    ...
    {
      title: 'FrontierCircuitTestHomeLanding',
      logging: true,
      storeDialog: true,
    },
  );
  ...
  // (mirror CadmiumLanding's `muxium.plan(...)` + `d.client.d.frontierCircuitTest.k.<prop>.select()` reads).
  ...
  // content into reactive refs. The N-watcher SMRP relay dispatches frontierCircuitTestSetDesignationMenuStage into
  // this page muxium; the selector flows the keyed Record in, and we read THIS page's designation key
  // (frontierCircuitTestName), falling back to EMPTY_MENU_STAGE (the default menu is supplied via :default-stage).
  // loadedDiamondContent / loadedOnyxContent feed the WTO RI widget (Tier-2 page-muxium reach · frontierCircuitTest
  // is mounted under client). Mirrors the cadmiumLandingSubscription staging/stage/d__ shape.
  muxium.plan<ClientMuxiumDeck>(
    'frontierCircuitTestMenuStageSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            const record = d.client.d.frontierCircuitTest.k.shatteriteMenus.select() as Record<string, MenuDocument>;
            menuDocument.value = record[frontierCircuitTestName.value] ?? EMPTY_MENU_DOCUMENT;
            loadedDiamondContent.value = d.client.d.frontierCircuitTest.k.loadedDiamondContent.select() as string;
            loadedOnyxContent.value = d.client.d.frontierCircuitTest.k.loadedOnyxContent.select() as string;
          },
          {
            selectors: [
              d__.client.d.frontierCircuitTest.k.shatteriteMenus,
              d__.client.d.frontierCircuitTest.k.loadedDiamondContent,
              d__.client.d.frontierCircuitTest.k.loadedOnyxContent,
            ],
          },
        ),
      ]),
  ...
  // The designation source is the frontierCircuitTestName ref (the ADAPT marker · Design Decision 3) — the SSMC
  // filter, this PAOLRP, and the ShatteriteMenu anchor-lookup ALL read frontierCircuitTestName (W3-A).
  //
  ...
  // frontierCircuitTestName). Runs once on mount; the Forge menu shows only while isUnactualized === true.
  ...
  // SCP's OFFLINE Forge sessions. Null on failure → the row matches by frontierCircuitTestName alone (safe superset).
  void (async () => {
    try {
...
<template>
  <div class="domain-landing">
    ...
    <!-- ============================================================
         ...
         widget + Forge menu + Shatterite Menu + Session Manager); Card = the FrontierCircuitTestCard by display
         name (the biplane-tab idiom · FrontierCircuitTestBiplane HOME|CARD standard).
         ============================================================ -->
    ...
    <main v-if="activeTab === 'home'" class="domain-content">
      ...
      <!-- ============================================================
           IE-D4 · WTO TRIPTYCH · ZONE 1 (1st) · the LIVE RI widget — FrontierCircuitTestCascadeDocs.
           Leads the triptych (context-first · user-binding order RI → Menu → Session Manager ·
           decision 4). Reads THIS designation's Cascades/Extended/<frontierCircuitTestName>/ Diamond+Onyx pair
           LIVE from the Tier-2 suiteCascade cascades Record (the extended auto-registration circuit) ·
           the plan pane is page-editable (/frontierCircuitTest-doc-save · DIAMOND-only) · the trajectory pane is
           ...
           ============================================================ -->
      <FrontierCircuitTestCascadeDocs :designation="frontierCircuitTestName" />
      ...
      <!-- ============================================================
           ...
           live keyed relay state (shatteriteMenus[frontierCircuitTestName]); :default-stage is the MINIMAL default
           menu (decision 5) the component renders when no live stage exists; :frontierCircuitTest-name binds the
      ...
      <div class="domain-menu-zone">
        <ShatteriteMenu
          ...
          :default-stage="FRONTIERCIRCUITTEST_DEFAULT_MENU_STAGE"
          :frontierCircuitTest-name="frontierCircuitTestName"
      ...
      <!-- ============================================================
           ...
           ADAPT: frontierCircuitTestName is already bound to the domain ref above — no edit needed here
      ...
      <ScsBridgeSessionManagement
        ...
        :mode="'specific'"
        :s8-name="frontierCircuitTestName"
      />
    ...
    <main v-else class="domain-content domain-card-page">
      <!-- W3 · THE CARD — the FrontierCircuitTestCard mounted by THIS page's display name (the biplane CARD tab). -->
      <FrontierCircuitTestCard
        :key="cardEntry.name"
        :entry="cardEntry"
        :domain="selfSnippet"
        :snippet="selfSnippet"
...
/* ============================================================
   W3 · THE ONE-BAR SUBNAV (D-EF-0) — the same-page tab idiom (FrontierCircuitTestBiplane bearing · zero raw hex).
   ============================================================ */
.domain-subnav {
  display: flex;
  gap: 0.5rem;
