...
/**
 * FrontierCircuitTestCard.vue — THE HERO PIPELINE CARD (MD-11 · D-EF-1 · the Entourage Forge Attend set)
 *
 ...
 *       resolvable image assets (probed from a candidate ref set via /frontierCircuitTest/:name/asset?ref=).
 *       The picked hero swaps the portrait; the picked background sets the card backdrop (a dark
 *       gradient wash so text holds). Picks PERSIST client-side (localStorage `frontierCircuitTest-card:<name>`).
 ...
 *       MD-4 GET /frontierCircuitTest/:name/skill/:skill route (pre · mono · scrollable · honest 404). One open
 ...
 * Citation: SCP-Origin FrontierCircuitTestSkillRow.vue / S8RDPanel.vue (the cell + hero-slot grid idiom · READ-ONLY).
 * Citation: frontierCircuitTestReaderPaths.model.ts (the MD-4 reader/asset routes this card consumes · D-RD-1/2).
...
// (C370-B): the string `s8` carries no `frontierCircuitTest`/`FrontierCircuitTest` token, so `frontierCircuitTest:page`'s copy-move-rename
// can NEVER rewrite it — a GENERATED page keeps a working reader family (the routes live in the
// shared `vue` concept, aliased under both /frontierCircuitTest AND /s8). This guard still stands belt-and-braces:
...
// ============================================
// PERSISTENCE · localStorage `frontierCircuitTest-card:<name>` (the honest first pass · server-side follow-up)
// ============================================
interface CardPrefs {
  hero?: string;
  background?: string;
}
const storageKey = computed<string>(() => `frontierCircuitTest-card:${props.entry.name}`);
...
// ============================================
// ASSET PROBE · the candidate ref set (/frontierCircuitTest/:name/asset?ref=<ref>)
// ============================================
...
<template>
  <article
    :class="['frontierCircuitTest-card', paneClass, { 'is-compact': compact, 'is-expanded': !compact }]"
...
.frontierCircuitTest-card.is-compact {
  cursor: pointer;
  min-height: 168px;
  transition: transform 0.18s ease, box-shadow 0.18s ease;
}

.frontierCircuitTest-card.is-compact:hover {
