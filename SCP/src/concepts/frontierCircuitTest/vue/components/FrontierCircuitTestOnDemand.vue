...
/**
 * FrontierCircuitTestOnDemand — On-Demand FrontierCircuitTest Spawn + First Message (ODSS · Band A-4)
 *
 ...
 * supplying a FrontierCircuitTest-specific MCP session endpoint (SAPR · A-3 plumbing).
 *
 * Invoke path (mirrors scsBridge MCP-tool-bindings-to-UI):
 *   1. User selects a Suite 8 from the `frontierCircuitTests` Record (DECK K · Tier 1).
 ...
 * FrontierCircuitTestEntry.name as the scpName arg to triggerSpawnSession so the bridge
 * calls setSessionFrontierCircuitTestName before spawn, routing Instance.md via
 * appendSystemPromptFile. Until then scpName=undefined → null → Template SCP
 * default (General Agent).
 *
 * Citation: MASTER-DIAMOND-FRONTIERCIRCUITTEST-CONCEPT-ASPIRANT.md §2 Band A-4 ODSS
...
// C1-D2 · SBST · the Suite 8 spawn lane uses isSpawningFrontierCircuitTest (independent SIGR guard).
import { isSpawningFrontierCircuitTest } from '../../../scsBridge/principles/scsBridgeInvokeSessionSpawn.principle.client';
...
const frontierCircuitTestsMap = ref<Record<string, FrontierCircuitTestEntry>>({});
...
onMounted(() => {
  ...
  muxium = createClientMuxiumInstance<ClientMuxiumDeck>(
    ...
    { title: 'FrontierCircuitTestOnDemand', logging: false, storeDialog: false },
  );
  ...
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'frontierCircuitTestOnDemandSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            frontierCircuitTestsMap.value = d.client.d.frontierCircuitTest.k.frontierCircuitTests.select();
          },
...
const selectedFrontierCircuitTestName = ref<string>('');
...
const frontierCircuitTestEntries = computed<FrontierCircuitTestEntry[]>(() =>
  Object.values(frontierCircuitTestsMap.value),
);

const hasEntries = computed<boolean>(() => frontierCircuitTestEntries.value.length > 0);
// SB-DS6 · Suite 8 entries mapped to the ScsDropdown {value,label} shape (value === label === name).
const frontierCircuitTestSelectOptions = computed(() =>
  frontierCircuitTestEntries.value.map((entry) => ({ value: entry.name, label: entry.name })),
);
...
const canFire = computed<boolean>(
  () =>
    bridgeReady.value &&
    hasEntries.value &&
    selectedFrontierCircuitTestName.value !== '' &&
    firstMessage.value.trim() !== '' &&
    !isSpawningFrontierCircuitTest.value &&
    phase.value === 'idle',
);
...
watch(
  ...
  async (newLen) => {
    ...
    console.log(
      '[FrontierCircuitTestOnDemand ODSS] Session arrived · id=', newest.id,
    ...
    if (result?.ok) {
      phase.value = 'done';
      console.log('[FrontierCircuitTestOnDemand ODSS] First message delivered · sessionId=', newest.id);
    } else {
      phase.value = 'error';
      errorMsg.value = result?.error ?? 'triggerSendMessage returned non-ok';
      console.error('[FrontierCircuitTestOnDemand ODSS] First message failed · error=', errorMsg.value);
    }
  },
);
...
function handleSpawnAndSend(): void {
  ...
  console.log(
    '[FrontierCircuitTestOnDemand ODSS] Spawn+Send · frontierCircuitTestName=', selectedFrontierCircuitTestName.value,
    '· messageLength=', firstMessage.value.trim().length,
    '· SBST: identified Suite 8 spawn (setSessionFrontierCircuitTestName BEFORE spawn)',
  );

  sessionCountAtSpawn.value = sessionsList.value.length;
  phase.value = 'spawning';

  // C1-D2 · SBST · identified Suite 8 spawn. The bridge tool sets entry.frontierCircuitTestName
  ...
  ctrl.triggerSpawnS8Session(selectedFrontierCircuitTestName.value);

  // SIGR auto-clear (mirrors ScsBridgeSessionManagement.vue:244 watch)
  // transitions phase → waiting once SIGR clears (isSpawningFrontierCircuitTest goes false).
  const sigRWatcher = watch(
    () => isSpawningFrontierCircuitTest.value,
    (nowSpawning) => {
      if (!nowSpawning && phase.value === 'spawning') {
        phase.value = 'waiting';
        console.log('[FrontierCircuitTestOnDemand ODSS] Spawn ack received · waiting for DSAB row');
  ...
  <div class="hifi-pane-base hifi-pane-blue odss-root">
    ...
    <div class="odss-field">
      ...
      <div v-if="!hasEntries" class="odss-empty-notice">
        No Suite 8 entries registered — frontierCircuitTests Record is empty (A-2 MPRF populates at boot).
      </div>
      <ScsDropdown
        v-else
        class="odss-select"
        ...
        :disabled="phase !== 'idle'"
        :options="frontierCircuitTestSelectOptions"
        v-model="selectedFrontierCircuitTestName"
      />
    </div>
