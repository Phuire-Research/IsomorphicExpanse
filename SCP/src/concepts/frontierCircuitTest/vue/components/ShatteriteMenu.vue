 ...
 * `frontierCircuitTestName` as props — exactly like STSC receives its `sfsd`. The page muxium owns the
 ...
 *   the page's Anchor = the alive session whose `frontierCircuitTestName === props.frontierCircuitTestName`
 ...
 *   onMounted from GET /frontierCircuitTest-anchor-spawn/<frontierCircuitTestName>), the inert "Waiting…" stub is replaced
 *   by a Pewter-styled "Spawn + Anchor this domain" button. handleSpawnAnchor spawns via the
 *   EXISTING triggerSpawnFrontierCircuitTestSession (the bridge's claimAnchorIfUnclaimed auto-anchors the page
 *   anchor), then a readiness poll (mirror FrontierCircuitTestHomeLanding PPOL · 250ms/3000ms) focuses the new
 ...
 *   the per-callsite page spawn buttons (FrontierCircuitTestHomeLanding · PewterLanding), making the menu
...
// the anchor-spawn route live in a NEVER-COPIED scsBridge model so the frontierCircuitTest:page token
// rewrite cannot break them in mints (s.frontierCircuitTestName → s.{domain}Name was the BO-1 kill).
...
// PAOLR · the page's Anchor for this frontierCircuitTestName — scoped to the FOCUSED citizen (D-AFS).
// Keys on isAnchor (authoritative page-bound session), NOT "any alive frontierCircuitTest match".
const anchor = computed<ScsBridgeSessionEntry | undefined>(() =>
  resolveS8Anchor(sessionsList.value, props.frontierCircuitTestName, focusedAnchorScpName.value),
);
...
// GET /frontierCircuitTest-anchor-spawn/<frontierCircuitTestName> (server resolves the per-Suite-8 Cascade.json ·
// DEFAULT 'prompt' on any failure · mirrors FrontierCircuitTestHomeLanding AD). 'prompt' = the menu surfaces a
...
// sidesteps the server anti-flood guard that a fresh spawn would trip). Mirrors FrontierCircuitTestHomeLanding /
...
// plan selector · mirrors FrontierCircuitTestHomeLanding PPOL-WUD).
...
// /frontierCircuitTest-anchor-spawn fetch can resolve BEFORE the offline prior hydrates → !anchor.value momentarily
// true → a spurious spawn that then steals the offline original via the fallback). This guard mirrors
// CadmiumLanding / FrontierCircuitTestHomeLanding PAOLR: settle over a window, then branch ONCE on the SETTLED
...
// component; no page wires its own floor). GET /frontierCircuitTest-menu/<frontierCircuitTestName> on mount → floorStage.
...
onMounted(() => {
  void fetch(s8MenuPath(props.frontierCircuitTestName))
...
const persistStageIndex = (idx: number): void => {
  void fetch(S8_MENU_STAGE_SET_PATH, {
    ...
    body: JSON.stringify({ designation: props.frontierCircuitTestName, currentStageIndex: idx }),
  }).catch(() => {
...
// /frontierCircuitTest-anchor-spawn/<frontierCircuitTestName> · DEFAULT 'prompt' on any failure · fire-and-forget · the
...
onMounted(() => {
  ...
  void fetch(s8AnchorSpawnPath(props.frontierCircuitTestName))
    ...
    .then((j: { anchorSpawn?: string }) => {
      ...
      // spawning over it (mirror CadmiumLanding / FrontierCircuitTestHomeLanding PAOLR three-branch).
...
// CadmiumLanding / FrontierCircuitTestHomeLanding PAOLR three-branch under a one-shot settle window: poll
...
function autoDecideAnchor(): void {
  ...
  autoDecidePoll = setInterval(() => {
    ...
    if (anchorAlive.value) {
      ...
      resolveOnce(() => {
        console.log('[ShatteriteMenu P1.1] AD-Auto · anchor ALIVE · no spawn · frontierCircuitTestName=', props.frontierCircuitTestName);
      });
      ...
      resolveOnce(() => {
        ...
        if (reengageTombstoned.value) {
          console.log('[ShatteriteMenu C472] anchor OFFLINE but TOMBSTONED · no auto re-engage · frontierCircuitTestName=', props.frontierCircuitTestName);
          return;
        }
        ...
        if (anchorSpawnMode.value !== 'auto') {
          console.log('[ShatteriteMenu C488] anchor OFFLINE · Auto-Spawn OFF · the button stands · frontierCircuitTestName=', props.frontierCircuitTestName);
          return;
        }
          ...
          void fetch(s8AnchorSpawnPath(props.frontierCircuitTestName))
            ...
            .then((j: { anchorSpawn?: string }) => {
              if (j.anchorSpawn === 'auto' && !spawning.value && !anchor.value) {
                console.log('[ShatteriteMenu P1.1] AD-Auto · NO anchor after settle · server-confirmed · spawn · frontierCircuitTestName=', props.frontierCircuitTestName);
                void handleSpawnAnchor();
              } else {
                console.log('[ShatteriteMenu C471] spawn double-check REFUSED · server mode=', j.anchorSpawn, '· frontierCircuitTestName=', props.frontierCircuitTestName);
              }
            })
            .catch(() => { /* unreachable server — never spawn blind */ });
        } else {
          console.log('[ShatteriteMenu C470] NO anchor after settle · auto-spawn OFF · the button stands · frontierCircuitTestName=', props.frontierCircuitTestName);
        }
      });
    }
  }, AD_STEP_MS);
...
async function toggleAutoMode(): Promise<void> {
  ...
  try {
    const res = await fetch(s8AnchorSpawnPath(props.frontierCircuitTestName), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const reengageTombstoneKey = (): string => `scsReengageTombstone:${props.frontierCircuitTestName}`;
...
watch(() => props.frontierCircuitTestName, (name, prev) => {
  if (name && !prev) {
...
watch([showSpawnOption, showReengageOption, anchorStatus], ([spawnOpt, reengOpt, status]) => {
  ...
  // (a cross-window set otherwise leaves this window's render stale until tab-in).
    ...
    '· spawning=', spawning.value, '· frontierCircuitTestName='.replace('frontierCircuitTestName','designation'), props.frontierCircuitTestName);
});
...
watch(settledAnchorStatus, (nowStatus, prevStatus) => {
  ...
  if (prevStatus === 'launched' && nowStatus === 'offline') {
    ...
    console.log('[ShatteriteMenu C472] anchor USER-CLOSED · tombstone SET · Auto-Spawn PRESERVED · frontierCircuitTestName=', props.frontierCircuitTestName);
  }
});

// SOE · spawn + anchor THIS domain from the menu (parallel to handleOption). Spawns via the EXISTING
// triggerSpawnFrontierCircuitTestSession (the bridge sets entry.frontierCircuitTestName BEFORE spawn → claimAnchorIfUnclaimed
// auto-anchors the page anchor), then a readiness poll (250ms/3000ms · mirror FrontierCircuitTestHomeLanding PPOL
// / CadmiumLanding PAOLRP) watches sessionsList for the launched anchor. On found → focus + clear.
// Fallback (a session for this frontierCircuitTestName appeared but did NOT auto-anchor): await triggerSetAnchor.
...
async function handleSpawnAnchor(): Promise<void> {
  ...
  spawning.value = true;
  console.log('[ShatteriteMenu SOE] spawn + anchor · frontierCircuitTestName=', props.frontierCircuitTestName);
  ...
  // C373 · triggerSpawnS8Session (rename-proof alias) — survives the frontierCircuitTest:page domain-token rewrite.
  const origin = await ensureOriginScpName();
  ctrl.triggerSpawnS8Session(props.frontierCircuitTestName, origin || undefined);
  ...
  spawnPoll = setInterval(() => {
    void (async () => {
      ...
      const live = sessions.find(
        (s) => s.frontierCircuitTestName === props.frontierCircuitTestName && matchesOwnCitizen(s) && !priorSessionIds.has(s.id) && s.isAnchor === true && s.status === 'launched',
      );
      if (live) {
      ...
      // auto-stamp. If a same-frontierCircuitTestName anchor ALREADY exists (any status — e.g. an offline prior
      ...
      if (elapsedMs >= SOE_MAX_MS) {
        if (spawnPoll) { clearInterval(spawnPoll); spawnPoll = null; }
        const existingAnchor = sessions.find((s) => s.frontierCircuitTestName === props.frontierCircuitTestName && matchesOwnCitizen(s) && s.isAnchor === true);
        ...
        const unanchored = sessions.find((s) => s.frontierCircuitTestName === props.frontierCircuitTestName && matchesOwnCitizen(s) && !priorSessionIds.has(s.id) && !s.isAnchor);
        if (existingAnchor) {
          ...
          console.warn('[ShatteriteMenu SOE] spawn settled with no session for frontierCircuitTestName=', props.frontierCircuitTestName);
        }
        spawning.value = false;
      }
    })();
...
// poll / FrontierCircuitTestHomeLanding PPOL) watches sessionsList for the anchor flipping to 'launched'. On found
...
async function handleReengageAnchor(): Promise<void> {
  ...
  console.log('[ShatteriteMenu P1] re-engage offline anchor · frontierCircuitTestName=', props.frontierCircuitTestName, '· anchorId=', target.id);
  ...
  reengagePoll = setInterval(() => {
    ...
    const live = sessionsList.value.find(
      (s) => s.frontierCircuitTestName === props.frontierCircuitTestName && matchesOwnCitizen(s) && s.isAnchor === true && s.status === 'launched',
    );
    if (live) {
    ...
    if (elapsedMs >= REENGAGE_MAX_MS) {
      if (reengagePoll) { clearInterval(reengagePoll); reengagePoll = null; }
      console.warn('[ShatteriteMenu P1] re-engage did not settle alive for frontierCircuitTestName=', props.frontierCircuitTestName);
      isReengaging.value = false;
    }
  }, REENGAGE_STEP_MS);
}
...
// (1) GET the SORD-wrapped Skill/Strategy envelope from /frontierCircuitTest-skill-prime, now wrapped
...
async function primeSend(
  ...
  const url = `/frontierCircuitTest-skill-prime/${encodeURIComponent(props.frontierCircuitTestName)}?ref=${encodeURIComponent(ref)}&kind=${primeKind}`;
...
async function handleOption(option: MenuOption): Promise<void> {
  ...
  const target = anchor.value;
  if (!ctrl || !target) {
    console.warn('[ShatteriteMenu SMSP] no alive Anchor for option · frontierCircuitTestName=', props.frontierCircuitTestName);
  ...
  try {
    if (option.kind === 'scs') {
      ...
      // server (/frontierCircuitTest-skill-prime), then relay the envelope to the live Anchor (triggerSendMessage)
...
async function handleSubmit(option: MenuOption, i: number): Promise<void> {
  ...
  if (typeof pairDirective === 'string' && pairDirective.length > 0) {
    ...
    const target = anchor.value;
    if (!ctrl || !target) {
      console.warn('[ShatteriteMenu TRP] no alive Anchor for pairing submit · frontierCircuitTestName=', props.frontierCircuitTestName);
      emit('option-selected', { label: option.label, kind: option.kind, ok: false });
      return;
    }
  ...
  const target = anchor.value;
  if (!ctrl || !target) {
    console.warn('[ShatteriteMenu MOIS] no alive Anchor for submit · frontierCircuitTestName=', props.frontierCircuitTestName);
    emit('option-selected', { label: option.label, kind: option.kind, ok: false });
    return;
  }
...
<template>
  ...
  >
    ...
    <div v-if="showSpawnOption" class="menu-stub menu-stub-spawn">
      <span class="menu-stub-text">No Anchor yet for {{ frontierCircuitTestName }} — engage to begin.</span>
      <button
        type="button"
    ...
    <!-- P1 · ORPHAN-ANCHOR RE-ENGAGE · an anchor exists but is offline (status !== 'launched').
         ...
         render (no extra wiring). Mirrors FrontierCircuitTestHomeLanding / CadmiumLanding PPOL branch (2). -->
    <div v-else-if="showReengageOption" class="menu-stub menu-stub-spawn">
      <span class="menu-stub-text">
        Anchor for {{ frontierCircuitTestName }} is offline — re-engage to bring it back online.
      </span>
      <button
        type="button"
        class="menu-option-btn menu-spawn-btn"
