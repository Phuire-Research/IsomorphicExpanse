 ...
 *
 * Citation: EPOCH-DIAMOND-FRONTIERCIRCUITTEST-SETUP-RESEARCH.md §2 Macro 1 SU
 ...
 * Citation: FrontierCircuitTestOnDemand.vue (triggerSendMessage await + controller computed)
 */
...
// PAOLR · the page's Anchor for this frontierCircuitTestName. Keys on isAnchor (authoritative
// page-bound session), NOT "any alive frontierCircuitTest match" — mirror CadmiumLanding.
const anchor = computed<ScsBridgeSessionEntry | undefined>(() =>
  sessionsList.value.find(
    (s) => s.frontierCircuitTestName === props.frontierCircuitTestName && s.isAnchor === true,
  ),
);
...
async function handleSubmit(): Promise<void> {
  ...
  console.log(
    '[ShatteriteTomeSetup STSC] handleSubmit · frontierCircuitTestName=', props.frontierCircuitTestName,
