...
/**
 * FrontierCircuitTestBiplane.vue — THE BIPLANE PAGE STANDARD (MD-6 · D-BP-1)
 *
 * TWO STARTING SUBPAGES for any Suite 8 detail surface, joined by a same-page HOME | CARD navbar
 * (the tab idiom the GitM sub-page uses · FrontierCircuitTestSubPageNav bearing). The standard the epoch cascades
 * onto every Suite 8 page (Cadmium · Pewter · SCS-Bridge · the minted default).
 *
 *   HOME subpage:
 *     (a) THE CASCADE DOCUMENTS — the /frontierCircuitTest/:name/working-docs list (file + firstLine); a click
 *         opens an in-page reader pane fetching /frontierCircuitTest/:name/working-doc?file=<file> (the MD-6
 ...
 *         control FrontierCircuitTestHomeLanding mounts · the per-NDEP anchor system · SAC anchorConfig). It owns
 ...
 *         Cadmium Researcher WITHOUT anchoring (triggerSpawnFrontierCircuitTestSession(name, null, asWorker=true) ·
 ...
 *
 *   CARD subpage: the MD-5 FrontierCircuitTestCard expanded (the full anatomy + the reader row) as the second
 *   starting subpage.
 *
 * Citation: DIAMOND-SCP-ACTUALIZATION-EPOCH.md §MD-6 (D-BP-1 · the two-subpage standard).
 * Citation: FrontierCircuitTestHomeLanding.vue (the ShatteriteMenu bound-anchor mount + PPOL idiom · reused).
 * Citation: FrontierCircuitTestSubPageNav.vue (the same-page HiFi tab navbar bearing · zero raw hex).
 * Citation: FrontierCircuitTestCard.vue (MD-5 · the expanded CARD subpage).
 */
...
import FrontierCircuitTestCard from './FrontierCircuitTestCard.vue';
// IE-D4e · THE FOUNDED-BUT-INVISIBLE FIX — the LIVE D-O pair (FrontierCircuitTestCascadeDocs) mounted BENEATH the
...
// auto-registration circuit + the two-roots /frontierCircuitTest-doc-tiers + /frontierCircuitTest-doc-save routes).
import FrontierCircuitTestCascadeDocs from './FrontierCircuitTestCascadeDocs.vue';
// MD-9 · D-MC-3 · Per-Instance Model Control · the Forge Actualization model selection (parity with
// FrontierCircuitTestHomeLanding's forge launch). The catalog mirror + the offscreen-safe in-DOM dropdown (native
...
// the bridge quality runs claimAnchorIfUnclaimed → the Forge binds THIS page's per-frontierCircuitTestName
...
<template>
  <section class="frontierCircuitTest-biplane">
    ...
    <div v-if="activeTab === 'home'" class="biplane-home">
      ...
      <div v-if="isUnactualized" class="forge-door">
        ...
        <!-- MD-9 · D-MC-3 · THE ACTUALIZATION MODEL SELECT — the user picks WHICH model the Forge's
             Actualization uses (parity with FrontierCircuitTestHomeLanding's forge launch). Seeded to Opus 4.8; the
      ...
      <FrontierCircuitTestCascadeDocs :designation="entry.name" />
      ...
      <div class="home-menu-zone">
        <ShatteriteMenu
          ...
          :frontierCircuitTest-name="entry.name"
...
<style scoped>
.frontierCircuitTest-biplane {
  display: flex;
  flex-direction: column;
  gap: 1rem;
}

/* THE NAVBAR — the same-page tab idiom (FrontierCircuitTestSubPageNav HiFi bearing · zero raw hex). */
.biplane-nav {
  display: flex;
  align-items: center;
