/**
 * FrontierCircuitTest Menu STCP Relay — Shared Config (GTMS8C · SD-6 single-source)
 *
 * ONE exported config object both `frontierCircuitTestMenuWatch` (the thin dir-watch arm) AND
 * `frontierCircuitTestMenuStcpRelay` (SMRP+BOCR) import — no duplicated config literals. Each principle
 * builds its OWN createStcpComponentRelay<MenuStage>(FRONTIERCIRCUITTEST_MENU_RELAY_CONFIG) INSTANCE
 ...
 * discipline so the path lands at Cascades/Extended/<DEFAULT_FRONTIERCIRCUITTEST_DESIGNATION_NAME>/menu.json.
 *
...
import { DEFAULT_FRONTIERCIRCUITTEST_DESIGNATION_NAME, FRONTIERCIRCUITTEST_MENU_JSON_BASENAME } from './frontierCircuitTest.type';
...
const FRONTIERCIRCUITTEST_MENU_JSON_PATH = path.resolve(
  SCS_ROOT,
  'Cascades',
  'Extended',
  DEFAULT_FRONTIERCIRCUITTEST_DESIGNATION_NAME,
  FRONTIERCIRCUITTEST_MENU_JSON_BASENAME,
);
...
const MENU_RELAY_SINK_PATH = path.resolve(SCS_ROOT, 'Cascades', 'Bridge', 'frontierCircuitTest-menu-relay.json');
...
export const parseMenuStage = (raw: string): MenuStage | null => {
  ...
  try {
    ...
    console.warn('[FrontierCircuitTest STCP menu] menu.json parse failed · ignoring (partial write?)');
    return null;
  }
  if (!parsed || typeof parsed !== 'object') return null;
  const p = parsed as Record<string, unknown>;
  if (typeof p.stageIndex !== 'number' || typeof p.title !== 'string' || !Array.isArray(p.options)) {
    console.warn('[FrontierCircuitTest STCP menu] menu.json missing required fields · ignoring');
    return null;
  }
...
// createFrontierCircuitTestDesignationRelayConfig (below) per registered designation instead.
export const FRONTIERCIRCUITTEST_MENU_RELAY_CONFIG: StcpComponentRelayConfig<MenuStage> = {
  jsonPath: FRONTIERCIRCUITTEST_MENU_JSON_PATH,
  basename: FRONTIERCIRCUITTEST_MENU_JSON_BASENAME,
  parsePayload: parseMenuStage,
  emptyPayload: EMPTY_MENU_STAGE,
  baseActionCreator: (menuStage) =>
    frontierCircuitTestSetMenuStageHuirthBase.actionCreator({ menuStage }) as AnyAction,
  relayActionCreator: (menuStage) =>
    frontierCircuitTestSetMenuStage.actionCreator({ menuStage }) as AnyAction,
  ...
  logTag: '[FrontierCircuitTest MenuWatch · STCP menu]',
...
const FRONTIERCIRCUITTEST_EXTENDED_ROOT_WALK_UP_LIMIT = 6;

const resolveDesignationExtendedRoot = (designation: string): string => {
  const candidateRoots: string[] = [SCS_ROOT];
  let dir = SCS_ROOT;
  for (let i = 0; i < FRONTIERCIRCUITTEST_EXTENDED_ROOT_WALK_UP_LIMIT; i += 1) {
    const parent = path.dirname(dir);
    if (parent === dir) break;
    dir = parent;
    candidateRoots.push(dir);
  }
  const logTag = `[FrontierCircuitTest MenuWatch · STCP menu · ${designation}]`;
...
export const createFrontierCircuitTestDesignationRelayConfig = (
  designation: string,
  ...
  return {
    ...
    baseActionCreator: (menuStage) =>
      frontierCircuitTestSetDesignationMenuStageHuirthBase.actionCreator({ designation, menuStage }) as AnyAction,
    relayActionCreator: (menuStage) =>
      frontierCircuitTestSetDesignationMenuStage.actionCreator({ designation, menuStage }) as AnyAction,
    payloadIdentity: (menuStage) => `${designation}:${menuStageContentIdentity(menuStage)}`,
    logTag: `[FrontierCircuitTest MenuWatch · STCP menu · ${designation}]`,
