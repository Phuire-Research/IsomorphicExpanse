/**
 * frontierCircuitTestMenuWatch.principle.huirth.ts — the N-designation menu.json dir-watch (PRE-EPOCH · WPS)
 *
 ...
 *
 * Designation source: KNOWN_FRONTIERCIRCUITTEST_ENTRIES (the MPRF seed · the SAME authoritative list that seeds
 * the client `frontierCircuitTests` Record at boot). The Huirth frontierCircuitTest concept carries NO `frontierCircuitTests` Record (it is
 ...
 * set is STILL seeded from KNOWN_FRONTIERCIRCUITTEST_ENTRIES (the MPRF seed), but a chokidar depth-0 `addDir` watch
 ...
 * Huirth `frontierCircuitTests` Record. A dir with no menu.json arms a watcher that hydrates the moment one lands.
 *
 * Each watcher's helper (createStcpComponentRelay<MenuDocument>(createFrontierCircuitTestDesignationRelayConfig(name)))
 * owns add/change→SBIS Base→Relay + unlink→JDIS Idle internally; the SBIS dispatch carries the
 * designation key (the keyed Base frontierCircuitTestSetDesignationMenuStageHuirthBase FIRST, then the keyed relay
 * frontierCircuitTestSetDesignationMenuStage). Dispatch is via `nextA` (async-safe action-queue append).
 *
 ...
 * Citation: frontierCircuitTestRegistration.model.ts KNOWN_FRONTIERCIRCUITTEST_ENTRIES (the boot designation seed).
 */
...
import { createFrontierCircuitTestDesignationRelayConfig } from '../frontierCircuitTestMenuRelay.config';
import { KNOWN_FRONTIERCIRCUITTEST_ENTRIES } from '../model/frontierCircuitTestRegistration.model';

export type FrontierCircuitTestMenuWatchDeck = MuxiumDeck & {
...
export type FrontierCircuitTestMenuWatchPrincipleType = PrincipleFunction<
  FrontierCircuitTestHuirthQualities,
  FrontierCircuitTestMenuWatchDeck,
  FrontierCircuitTestHuirthState
>;
...
const MENU_WATCH_SINK_PATH = path.resolve(
  ...
  'Cascades',
  'Bridge',
  'frontierCircuitTest-menu-watch.json',
);
...
export const frontierCircuitTestMenuWatchPrinciple: FrontierCircuitTestMenuWatchPrincipleType = ({ plan, nextA }) => {
  console.log('[FrontierCircuitTest MenuWatch] Principle started · N-designation menu.json dir-watch (WPS)');
  ...
  const armDesignation = (designation: string): void => {
    ...
    try {
      const menuRelay = createStcpComponentRelay<MenuDocument>(
        createFrontierCircuitTestDesignationRelayConfig(trimmed),
      );
      ...
      if (watcher) {
        ...
        console.log('[FrontierCircuitTest MenuWatch] armed designation ·', trimmed);
        sinkMenuWatchTelemetry('arm-designation', { designation: trimmed });
      }
    } catch (err) {
      console.log('[FrontierCircuitTest MenuWatch] arm.skip · reason=arm-failed · designation=', trimmed, '·', err);
  ...
  const readExtendedSubdirectoryNames = (): string[] => {
    try {
      ...
      console.log(
        '[FrontierCircuitTest MenuWatch] extended.enumerate.skip · reason=',
  ...
  const menuWatchPlan = plan('FrontierCircuitTest MenuWatch (Huirth · N-designation dir-watch · WPS)', ({ stage }) => [
    ...
    stage(
      ({ d, dispatch }) => {
        console.log('[FrontierCircuitTest MenuWatch] Stage 1 · arm N menu.json dir-watches');
        // Boot seed — the MPRF authoritative list.
        for (const entry of KNOWN_FRONTIERCIRCUITTEST_ENTRIES) {
        ...
        console.log('[FrontierCircuitTest MenuWatch] armed', watcherMap.size, 'designation watcher(s)');
        ...
        try {
          ...
          const handleAddDir = (): void => {
            ...
            addDirDebounceTimer = setTimeout(() => {
              console.log('[FrontierCircuitTest MenuWatch] addDir · re-sweeping Extended for new designations');
          ...
          extendedDirWatcher.on('error', (err) => {
            console.log('[FrontierCircuitTest MenuWatch] watch.skip · reason=chokidar-error ·', err);
          });
          console.log('[FrontierCircuitTest MenuWatch] chokidar armed on', EXTENDED_ROOT_ABS, '· depth=0 · addDir');
          sinkMenuWatchTelemetry('live-adddir-armed', { extendedRoot: EXTENDED_ROOT_ABS });
        } catch (err) {
          console.log('[FrontierCircuitTest MenuWatch] watch.skip · reason=arm-failed ·', err);
  ...
  return () => {
    console.log('[FrontierCircuitTest MenuWatch] Principle cleanup · closing', watcherMap.size, 'watcher(s)');
