/**
 * IsomorphicExpanse Menu STCP Relay — Shared Config (GTMS8C · SD-6 single-source)
 *
 * ONE exported config object both `isomorphicExpanseMenuWatch` (the thin dir-watch arm) AND
 * `isomorphicExpanseMenuStcpRelay` (SMRP+BOCR) import — no duplicated config literals. Each principle
 * builds its OWN createStcpComponentRelay<MenuStage>(ISOMORPHICEXPANSE_MENU_RELAY_CONFIG) INSTANCE
 ...
 * discipline so the path lands at Cascades/Extended/<DEFAULT_ISOMORPHICEXPANSE_DESIGNATION_NAME>/menu.json.
 *
...
import { DEFAULT_ISOMORPHICEXPANSE_DESIGNATION_NAME, ISOMORPHICEXPANSE_MENU_JSON_BASENAME } from './isomorphicExpanse.type';
import { isomorphicExpanseSetMenuStage } from './qualities/isomorphicExpanseSetMenuStage.quality.client';
import { isomorphicExpanseSetMenuStageHuirthBase } from './qualities/isomorphicExpanseSetMenuStageHuirthBase.quality.huirth';
...
import { isomorphicExpanseSetDesignationMenuStageHuirthBase } from './qualities/isomorphicExpanseSetDesignationMenuStageHuirthBase.quality.huirth';
...
const ISOMORPHICEXPANSE_MENU_JSON_PATH = path.resolve(
  SCS_ROOT,
  'Cascades',
  'Extended',
  DEFAULT_ISOMORPHICEXPANSE_DESIGNATION_NAME,
  ISOMORPHICEXPANSE_MENU_JSON_BASENAME,
);
...
const MENU_RELAY_SINK_PATH = path.resolve(SCS_ROOT, 'Cascades', 'Bridge', 'isomorphicExpanse-menu-relay.json');
...
export const parseMenuStage = (raw: string): MenuStage | null => {
  ...
  try {
    ...
    console.warn('[IsomorphicExpanse STCP menu] menu.json parse failed · ignoring (partial write?)');
    return null;
  }
  if (!parsed || typeof parsed !== 'object') return null;
  const p = parsed as Record<string, unknown>;
  if (typeof p.stageIndex !== 'number' || typeof p.title !== 'string' || !Array.isArray(p.options)) {
    console.warn('[IsomorphicExpanse STCP menu] menu.json missing required fields · ignoring');
    return null;
  }
...
// createIsomorphicExpanseDesignationRelayConfig (below) per registered designation instead.
export const ISOMORPHICEXPANSE_MENU_RELAY_CONFIG: StcpComponentRelayConfig<MenuStage> = {
  jsonPath: ISOMORPHICEXPANSE_MENU_JSON_PATH,
  basename: ISOMORPHICEXPANSE_MENU_JSON_BASENAME,
  parsePayload: parseMenuStage,
  emptyPayload: EMPTY_MENU_STAGE,
  baseActionCreator: (menuStage) =>
    isomorphicExpanseSetMenuStageHuirthBase.actionCreator({ menuStage }) as AnyAction,
  relayActionCreator: (menuStage) =>
    isomorphicExpanseSetMenuStage.actionCreator({ menuStage }) as AnyAction,
  ...
  logTag: '[IsomorphicExpanse MenuWatch · STCP menu]',
...
const ISOMORPHICEXPANSE_EXTENDED_ROOT_WALK_UP_LIMIT = 6;

const resolveDesignationExtendedRoot = (designation: string): string => {
  const candidateRoots: string[] = [SCS_ROOT];
  let dir = SCS_ROOT;
  for (let i = 0; i < ISOMORPHICEXPANSE_EXTENDED_ROOT_WALK_UP_LIMIT; i += 1) {
    const parent = path.dirname(dir);
    if (parent === dir) break;
    dir = parent;
    candidateRoots.push(dir);
  }
  const logTag = `[IsomorphicExpanse MenuWatch · STCP menu · ${designation}]`;
...
export const createIsomorphicExpanseDesignationRelayConfig = (
  designation: string,
  ...
  return {
    ...
    baseActionCreator: (menuStage) =>
      isomorphicExpanseSetDesignationMenuStageHuirthBase.actionCreator({ designation, menuStage }) as AnyAction,
    relayActionCreator: (menuStage) =>
      isomorphicExpanseSetDesignationMenuStage.actionCreator({ designation, menuStage }) as AnyAction,
    payloadIdentity: (menuStage) => `${designation}:${menuStageContentIdentity(menuStage)}`,
    logTag: `[IsomorphicExpanse MenuWatch · STCP menu · ${designation}]`,
