/**
 * IsomorphicExpanse Default Menu Model File — the MINIMAL standing Shatterite Menu (PRE-EPOCH · decision 5)
 *
 * Pure model: zero Stratimux imports, zero dispatch. The DEFAULT MenuStage the IsomorphicExpanseHomeLanding
 * passes to ShatteriteMenu as its `defaultStage` prop. When NO live agent-authored menu.json stage
 * exists for this page's designation (shatteriteMenus[isomorphicExpanseName] absent or EMPTY_MENU_STAGE), the
 ...
 *   - The spawn-anchor row is RENDERED AT THE PAGE LEVEL (IsomorphicExpanseHomeLanding · the AD `'prompt'`
 ...
 *     alive); the actual first-load spawn path is the page prompt-row → triggerSpawnIsomorphicExpanseSession.
 ...
 * Citation: isomorphicExpanseCreateScaffold.model.ts buildFoundingVermillionCommand (the founding shape).
...
const ISOMORPHICEXPANSE_DEFAULT_FOUNDING_COMMAND = [
...
export const ISOMORPHICEXPANSE_DEFAULT_MENU_STAGE: MenuStage = {
  stageIndex: 0,
  ...
  options: [
    {
      ...
      // affordance (triggerSpawnIsomorphicExpanseSession); this menu row is the standing explainer for it.
      ...
      label: 'Summon Anchor',
      kind: 'askMore',
      scsCommand: ISOMORPHICEXPANSE_DEFAULT_FOUNDING_COMMAND,
