/**
 * isomorphicExpanseMenuWatch.principle.huirth.ts — the N-designation menu.json dir-watch (PRE-EPOCH · WPS)
 *
 ...
 *
 * Designation source: KNOWN_ISOMORPHICEXPANSE_ENTRIES (the MPRF seed · the SAME authoritative list that seeds
 * the client `isomorphicExpanses` Record at boot). The Huirth isomorphicExpanse concept carries NO `isomorphicExpanses` Record (it is
 ...
 * set is STILL seeded from KNOWN_ISOMORPHICEXPANSE_ENTRIES (the MPRF seed), but a chokidar depth-0 `addDir` watch
 ...
 * Huirth `isomorphicExpanses` Record. A dir with no menu.json arms a watcher that hydrates the moment one lands.
 *
 * Each watcher's helper (createStcpComponentRelay<MenuDocument>(createIsomorphicExpanseDesignationRelayConfig(name)))
 * owns add/change→SBIS Base→Relay + unlink→JDIS Idle internally; the SBIS dispatch carries the
 * designation key (the keyed Base isomorphicExpanseSetDesignationMenuStageHuirthBase FIRST, then the keyed relay
 * isomorphicExpanseSetDesignationMenuStage). Dispatch is via `nextA` (async-safe action-queue append).
 *
 ...
 * Citation: isomorphicExpanseRegistration.model.ts KNOWN_ISOMORPHICEXPANSE_ENTRIES (the boot designation seed).
 */
...
import type {
  ...
  IsomorphicExpanseHuirthQualities,
...
import { createIsomorphicExpanseDesignationRelayConfig } from '../isomorphicExpanseMenuRelay.config';
import { KNOWN_ISOMORPHICEXPANSE_ENTRIES } from '../model/isomorphicExpanseRegistration.model';

export type IsomorphicExpanseMenuWatchDeck = MuxiumDeck & {
  isomorphicExpanse: Concept<IsomorphicExpanseHuirthState, IsomorphicExpanseHuirthQualities>;
  webSocketServer: Concept<WebSocketServerState, WebSocketServerQualities>;
};

export type IsomorphicExpanseMenuWatchPrincipleType = PrincipleFunction<
  IsomorphicExpanseHuirthQualities,
  IsomorphicExpanseMenuWatchDeck,
  IsomorphicExpanseHuirthState
>;
...
const MENU_WATCH_SINK_PATH = path.resolve(
  ...
  'Cascades',
  'Bridge',
  'isomorphicExpanse-menu-watch.json',
);
...
export const isomorphicExpanseMenuWatchPrinciple: IsomorphicExpanseMenuWatchPrincipleType = ({ plan, nextA }) => {
  console.log('[IsomorphicExpanse MenuWatch] Principle started · N-designation menu.json dir-watch (WPS)');
  ...
  const armDesignation = (designation: string): void => {
    ...
    try {
      const menuRelay = createStcpComponentRelay<MenuDocument>(
        createIsomorphicExpanseDesignationRelayConfig(trimmed),
      );
      ...
      if (watcher) {
        ...
        console.log('[IsomorphicExpanse MenuWatch] armed designation ·', trimmed);
        sinkMenuWatchTelemetry('arm-designation', { designation: trimmed });
      }
    } catch (err) {
      console.log('[IsomorphicExpanse MenuWatch] arm.skip · reason=arm-failed · designation=', trimmed, '·', err);
  ...
  const readExtendedSubdirectoryNames = (): string[] => {
    try {
      ...
      console.log(
        '[IsomorphicExpanse MenuWatch] extended.enumerate.skip · reason=',
  ...
  const menuWatchPlan = plan('IsomorphicExpanse MenuWatch (Huirth · N-designation dir-watch · WPS)', ({ stage }) => [
    ...
    stage(
      ({ d, dispatch }) => {
        console.log('[IsomorphicExpanse MenuWatch] Stage 1 · arm N menu.json dir-watches');
        // Boot seed — the MPRF authoritative list.
        for (const entry of KNOWN_ISOMORPHICEXPANSE_ENTRIES) {
        ...
        console.log('[IsomorphicExpanse MenuWatch] armed', watcherMap.size, 'designation watcher(s)');
        ...
        try {
          ...
          const handleAddDir = (): void => {
            ...
            addDirDebounceTimer = setTimeout(() => {
              console.log('[IsomorphicExpanse MenuWatch] addDir · re-sweeping Extended for new designations');
          ...
          extendedDirWatcher.on('error', (err) => {
            console.log('[IsomorphicExpanse MenuWatch] watch.skip · reason=chokidar-error ·', err);
          });
          console.log('[IsomorphicExpanse MenuWatch] chokidar armed on', EXTENDED_ROOT_ABS, '· depth=0 · addDir');
          sinkMenuWatchTelemetry('live-adddir-armed', { extendedRoot: EXTENDED_ROOT_ABS });
        } catch (err) {
          console.log('[IsomorphicExpanse MenuWatch] watch.skip · reason=arm-failed ·', err);
  ...
  return () => {
    console.log('[IsomorphicExpanse MenuWatch] Principle cleanup · closing', watcherMap.size, 'watcher(s)');
