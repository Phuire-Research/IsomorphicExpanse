/**
 * isomorphicExpanseMenuStcpRelay Principle (PRE-EPOCH · BSSM keyed · STCP · SMRP + BOCR · Huirth)
 *
 * The keyed-Record generalization of the scalar live-relay stack. The two halves of the proven
 * live-relay pattern, now applied to the isomorphicExpanse keyed `shatteriteMenus` Record:
 *
 *   SMRP (State-Mirror-Reactive-Principle) — selector-reactive on d.isomorphicExpanse.k.shatteriteMenus (the
 *     WHOLE Record · any-key change fires). On every Base change (the N-watcher dir-watch dispatches
 *     isomorphicExpanseSetDesignationMenuStageHuirthBase FIRST), diff the Record against the last-known snapshot
 *     to find WHICH designation(s) changed, then broadcast the keyed relay (isomorphicExpanseSetDesignationMenuStage)
 ...
 *
 * Deck: MuxiumDeck & { isomorphicExpanse: IsomorphicExpanseHuirthConcept; webSocketServer }. Reads d.isomorphicExpanse.k.shatteriteMenus
...
import { createIsomorphicExpanseDesignationRelayConfig, menuStageContentIdentity } from '../isomorphicExpanseMenuRelay.config';

export type IsomorphicExpanseMenuStcpRelayDeck = MuxiumDeck & {
...
export type IsomorphicExpanseMenuStcpRelayPrincipleType = PrincipleFunction<
  IsomorphicExpanseHuirthQualities,
  IsomorphicExpanseMenuStcpRelayDeck,
  IsomorphicExpanseHuirthState
>;

export const isomorphicExpanseMenuStcpRelayPrinciple: IsomorphicExpanseMenuStcpRelayPrincipleType = ({ d_, plan }) => {
  console.log('[IsomorphicExpanse STCP Relay] Principle started · keyed SMRP + BOCR (Huirth)');
  ...
  const relayFor = (designation: string) => {
    ...
    if (!r) {
      r = createStcpComponentRelay<MenuDocument>(createIsomorphicExpanseDesignationRelayConfig(designation));
  ...
  const relayPlan = plan('IsomorphicExpanse Menu STCP Relay (keyed SMRP + BOCR · Huirth)', ({ stage }) => [
    // SMRP — selector-reactive on isomorphicExpanse.k.shatteriteMenus (whole Record) → diff → broadcast changed.
    stage(
      ({ d, dispatch }) => {
        const record = d.isomorphicExpanse.k.shatteriteMenus.select() as Record<string, MenuDocument>;
        ...
        for (const [designation, stageVal] of Object.entries(record)) {
          ...
          console.log(
            '[IsomorphicExpanse STCP Relay] SMRP · broadcast', designation, '· currentStageIndex=', stageVal.currentStageIndex,
          );
        }
      },
      {
        selectors: [d_.isomorphicExpanse.k.shatteriteMenus],
        beat: 1,
      },
    ),
    ...
    stage(
      ({ d, dispatch }) => {
        ...
        console.log(
          '[IsomorphicExpanse STCP Relay] BOCR · backfilled', newIds.length, 'client(s) ·',
  ...
  return () => {
    console.log('[IsomorphicExpanse STCP Relay] Principle cleanup');
    relayPlan.conclude();
  };
};
