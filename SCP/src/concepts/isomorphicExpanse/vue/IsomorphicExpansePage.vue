...
import { ISOMORPHICEXPANSE_TABS, type IsomorphicExpanseDesignation, type IsomorphicExpanseTab } from '../isomorphicExpanse.type';
// Cycle 159 D1 · IUPA · isomorphicExpanse muxified per-page (no longer base)
...
const designations = ref<IsomorphicExpanseDesignation[]>([]);
const activeDesignationName = ref<string>('');
const activeTab = ref<IsomorphicExpanseTab>('info');
...
const activeDesignation = computed<IsomorphicExpanseDesignation | null>(() => {
...
function switchTab(tab: IsomorphicExpanseTab) {
  if (!muxium) return;
  muxium.dispatch(
    (muxium as Muxium<ClientMuxiumDeck>).deck.d.client.d.isomorphicExpanse.e.isomorphicExpanseSetActiveTab({ tab }),
  );
}
...
onMounted(() => {
  ...
  // Cycle 159 D1 · IUPA · landing supplies isomorphicExpanse as muxonomic page concept
  ...
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'isomorphicExpansePageSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            designations.value = d.client.d.isomorphicExpanse.k.designations.select();
            activeDesignationName.value = d.client.d.isomorphicExpanse.k.activeDesignationName.select();
            activeTab.value = d.client.d.isomorphicExpanse.k.activeTab.select();
            loadedDiamondContent.value = d.client.d.isomorphicExpanse.k.loadedDiamondContent.select();
            loadedOnyxContent.value = d.client.d.isomorphicExpanse.k.loadedOnyxContent.select();
            loadedBoundCascade.value = d.client.d.isomorphicExpanse.k.loadedBoundCascade.select();
            loadedFileSystemSheet.value = d.client.d.isomorphicExpanse.k.loadedFileSystemSheet.select();
          },
          {
            selectors: [
              d__.client.d.isomorphicExpanse.k.designations,
              d__.client.d.isomorphicExpanse.k.activeDesignationName,
              d__.client.d.isomorphicExpanse.k.activeTab,
              d__.client.d.isomorphicExpanse.k.loadedDiamondContent,
              d__.client.d.isomorphicExpanse.k.loadedOnyxContent,
              d__.client.d.isomorphicExpanse.k.loadedBoundCascade,
              d__.client.d.isomorphicExpanse.k.loadedFileSystemSheet,
            ],
          },
        ),
      ]),
...
<template>
  <div class="isomorphicExpanse-page">
    ...
    <nav class="tab-nav">
      <button
        v-for="tab in ISOMORPHICEXPANSE_TABS"
        :key="tab.value"
    ...
    <main class="tab-content">
      <section v-if="activeTab === 'info'" class="info-tab">
        ...
        <p v-else class="placeholder">
          (info-sheet not loaded — register sample designations from IsomorphicExpanseLanding for visual preview)
        </p>
      </section>
      ...
      <section v-else-if="activeTab === 'doviewer'" class="doviewer-tab">
        ...
        <div class="doviewer-grid">
          <div class="doviewer-pane">
            ...
            <p v-else class="placeholder">
              (Diamond not loaded — register sample designations from IsomorphicExpanseLanding)
            </p>
          </div>
          <div class="doviewer-pane">
            <h3>Onyx (Lambda)</h3>
            ...
            <p v-else class="placeholder">
              (Onyx not loaded — register sample designations from IsomorphicExpanseLanding)
            </p>
          </div>
        </div>
      </section>
