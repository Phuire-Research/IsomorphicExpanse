...
/**
 * IsomorphicExpanseBiplane.vue — THE BIPLANE PAGE STANDARD (MD-6 · D-BP-1)
 *
 * TWO STARTING SUBPAGES for any Suite 8 detail surface, joined by a same-page HOME | CARD navbar
 * (the tab idiom the GitM sub-page uses · IsomorphicExpanseSubPageNav bearing). The standard the epoch cascades
 * onto every Suite 8 page (Cadmium · Pewter · SCS-Bridge · the minted default).
 *
 *   HOME subpage:
 *     (a) THE CASCADE DOCUMENTS — the /isomorphicExpanse/:name/working-docs list (file + firstLine); a click
 *         opens an in-page reader pane fetching /isomorphicExpanse/:name/working-doc?file=<file> (the MD-6
 ...
 *         control IsomorphicExpanseHomeLanding mounts · the per-NDEP anchor system · SAC anchorConfig). It owns
 ...
 *         Cadmium Researcher WITHOUT anchoring (triggerSpawnIsomorphicExpanseSession(name, null, asWorker=true) ·
 ...
 *
 *   CARD subpage: the MD-5 IsomorphicExpanseCard expanded (the full anatomy + the reader row) as the second
 *   starting subpage.
 *
 * Citation: DIAMOND-SCP-ACTUALIZATION-EPOCH.md §MD-6 (D-BP-1 · the two-subpage standard).
 * Citation: IsomorphicExpanseHomeLanding.vue (the ShatteriteMenu bound-anchor mount + PPOL idiom · reused).
 * Citation: IsomorphicExpanseSubPageNav.vue (the same-page HiFi tab navbar bearing · zero raw hex).
 * Citation: IsomorphicExpanseCard.vue (MD-5 · the expanded CARD subpage).
 */
...
import IsomorphicExpanseCard from './IsomorphicExpanseCard.vue';
// MD-9 · D-MC-3 · Per-Instance Model Control · the Forge Actualization model selection (parity with
// IsomorphicExpanseHomeLanding's forge launch). The catalog mirror + the offscreen-safe in-DOM dropdown (native
...
// the bridge quality runs claimAnchorIfUnclaimed → the Forge binds THIS page's per-isomorphicExpanseName
...
async function engageEntourageForge() {
  ...
  try {
    ...
    const liveForge = (ctrl.sessionsList.value ?? []).find(
      (s) =>
        s.suite8Name === 'Entourage Forge' &&
...
// mortal. resolvedScpName is filled on mount (best-effort · null → match by isomorphicExpanseName alone).
// ============================================================
...
onMounted(() => {
  ...
  // SCP's OFFLINE Forge sessions. Null on failure → the row matches by isomorphicExpanseName alone (safe superset).
  void (async () => {
    try {
...
<template>
  <section class="isomorphicExpanse-biplane">
    ...
    <div v-if="activeTab === 'home'" class="biplane-home">
      ...
      <div v-if="isUnactualized" class="forge-door">
        ...
        <!-- MD-9 · D-MC-3 · THE ACTUALIZATION MODEL SELECT — the user picks WHICH model the Forge's
             Actualization uses (parity with IsomorphicExpanseHomeLanding's forge launch). Seeded to Opus 4.8; the
      ...
      <div class="home-menu-zone">
        <ShatteriteMenu
          ...
          :isomorphicExpanse-name="entry.name"
...
<style scoped>
.isomorphicExpanse-biplane {
  display: flex;
  flex-direction: column;
  gap: 1rem;
}

/* THE NAVBAR — the same-page tab idiom (IsomorphicExpanseSubPageNav HiFi bearing · zero raw hex). */
.biplane-nav {
  display: flex;
  align-items: center;
