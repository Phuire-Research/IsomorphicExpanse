...
/**
 * IsomorphicExpanse Sub-Page Navigation (Band A-6 HCD)
 *
 ...
 * click; the parent dispatches the Stratimux Quality (isomorphicExpanseSetActiveSubPage).
 *
 ...
 * Citation: isomorphicExpanse.subPageRegistry.ts (ISOMORPHICEXPANSE_SUB_PAGE_OPTIONS source).
 */
import type { IsomorphicExpanseSubPage } from '../../isomorphicExpanse.type';
import type { IsomorphicExpanseSubPageOption } from '../../isomorphicExpanse.subPageRegistry';

interface Props {
  options: IsomorphicExpanseSubPageOption[];
...
const emit = defineEmits<{
  (event: 'subPageSelected', value: IsomorphicExpanseSubPage): void;
}>();

function handleClick(option: IsomorphicExpanseSubPageOption) {
...
<template>
  <nav class="isomorphicExpanse-subpage-nav hifi-pane-base">
