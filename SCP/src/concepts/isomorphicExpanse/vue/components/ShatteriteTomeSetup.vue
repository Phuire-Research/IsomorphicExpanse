 ...
 *
 * Citation: EPOCH-DIAMOND-ISOMORPHICEXPANSE-SETUP-RESEARCH.md §2 Macro 1 SU
 ...
 * Citation: IsomorphicExpanseOnDemand.vue (triggerSendMessage await + controller computed)
 */
...
// page-bound session), NOT "any alive isomorphicExpanse match" — mirror CadmiumLanding.
const anchor = computed<ScsBridgeSessionEntry | undefined>(() =>
  sessionsList.value.find(
    (s) => s.isomorphicExpanseName === props.isomorphicExpanseName && s.isAnchor === true,
  ),
);
...
async function handleSubmit(): Promise<void> {
  ...
  console.log(
    '[ShatteriteTomeSetup STSC] handleSubmit · isomorphicExpanseName=', props.isomorphicExpanseName,
