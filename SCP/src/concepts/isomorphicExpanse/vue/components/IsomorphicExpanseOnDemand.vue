...
/**
 * IsomorphicExpanseOnDemand — On-Demand IsomorphicExpanse Spawn + First Message (ODSS · Band A-4)
 *
 ...
 * supplying a IsomorphicExpanse-specific MCP session endpoint (SAPR · A-3 plumbing).
 *
 * Invoke path (mirrors scsBridge MCP-tool-bindings-to-UI):
 *   1. User selects a Suite 8 from the `isomorphicExpanses` Record (DECK K · Tier 1).
 ...
 * IsomorphicExpanseEntry.name as the scpName arg to triggerSpawnSession so the bridge
 * calls setSessionIsomorphicExpanseName before spawn, routing Instance.md via
 * appendSystemPromptFile. Until then scpName=undefined → null → Template SCP
 * default (General Agent).
 *
 * Citation: MASTER-DIAMOND-ISOMORPHICEXPANSE-CONCEPT-ASPIRANT.md §2 Band A-4 ODSS
...
// C1-D2 · SBST · the Suite 8 spawn lane uses isSpawningIsomorphicExpanse (independent SIGR guard).
import { isSpawningIsomorphicExpanse } from '../../../scsBridge/principles/scsBridgeInvokeSessionSpawn.principle.client';
...
const isomorphicExpansesMap = ref<Record<string, IsomorphicExpanseEntry>>({});
...
onMounted(() => {
  ...
  muxium = createClientMuxiumInstance<ClientMuxiumDeck>(
    ...
    { title: 'IsomorphicExpanseOnDemand', logging: false, storeDialog: false },
  );
  ...
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'isomorphicExpanseOnDemandSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            isomorphicExpansesMap.value = d.client.d.isomorphicExpanse.k.isomorphicExpanses.select();
          },
...
const selectedIsomorphicExpanseName = ref<string>('');
...
const isomorphicExpanseEntries = computed<IsomorphicExpanseEntry[]>(() =>
  Object.values(isomorphicExpansesMap.value),
);

const hasEntries = computed<boolean>(() => isomorphicExpanseEntries.value.length > 0);
// SB-DS6 · Suite 8 entries mapped to the ScsDropdown {value,label} shape (value === label === name).
const isomorphicExpanseSelectOptions = computed(() =>
  isomorphicExpanseEntries.value.map((entry) => ({ value: entry.name, label: entry.name })),
);
...
const canFire = computed<boolean>(
  () =>
    bridgeReady.value &&
    hasEntries.value &&
    selectedIsomorphicExpanseName.value !== '' &&
    firstMessage.value.trim() !== '' &&
    !isSpawningIsomorphicExpanse.value &&
    phase.value === 'idle',
);
...
watch(
  ...
  async (newLen) => {
    ...
    console.log(
      '[IsomorphicExpanseOnDemand ODSS] Session arrived · id=', newest.id,
    ...
    if (result?.ok) {
      phase.value = 'done';
      console.log('[IsomorphicExpanseOnDemand ODSS] First message delivered · sessionId=', newest.id);
    } else {
      phase.value = 'error';
      errorMsg.value = result?.error ?? 'triggerSendMessage returned non-ok';
      console.error('[IsomorphicExpanseOnDemand ODSS] First message failed · error=', errorMsg.value);
    }
  },
);
...
function handleSpawnAndSend(): void {
  ...
  console.log(
    '[IsomorphicExpanseOnDemand ODSS] Spawn+Send · isomorphicExpanseName=', selectedIsomorphicExpanseName.value,
    '· messageLength=', firstMessage.value.trim().length,
    '· SBST: identified Suite 8 spawn (setSessionIsomorphicExpanseName BEFORE spawn)',
  );

  sessionCountAtSpawn.value = sessionsList.value.length;
  phase.value = 'spawning';

  // C1-D2 · SBST · identified Suite 8 spawn. The bridge tool sets entry.isomorphicExpanseName
  ...
  ctrl.triggerSpawnS8Session(selectedIsomorphicExpanseName.value);

  // SIGR auto-clear (mirrors ScsBridgeSessionManagement.vue:244 watch)
  // transitions phase → waiting once SIGR clears (isSpawningIsomorphicExpanse goes false).
  const sigRWatcher = watch(
    () => isSpawningIsomorphicExpanse.value,
    (nowSpawning) => {
      if (!nowSpawning && phase.value === 'spawning') {
        phase.value = 'waiting';
        console.log('[IsomorphicExpanseOnDemand ODSS] Spawn ack received · waiting for DSAB row');
  ...
  <div class="hifi-pane-base hifi-pane-blue odss-root">
    ...
    <div class="odss-field">
      ...
      <div v-if="!hasEntries" class="odss-empty-notice">
        No Suite 8 entries registered — isomorphicExpanses Record is empty (A-2 MPRF populates at boot).
      </div>
      <ScsDropdown
        v-else
        class="odss-select"
        ...
        :disabled="phase !== 'idle'"
        :options="isomorphicExpanseSelectOptions"
        v-model="selectedIsomorphicExpanseName"
      />
    </div>
