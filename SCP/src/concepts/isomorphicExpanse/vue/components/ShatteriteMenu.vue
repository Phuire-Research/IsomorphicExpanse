 ...
 * `isomorphicExpanseName` as props — exactly like STSC receives its `sfsd`. The page muxium owns the
 ...
 *   the page's Anchor = the alive session whose `isomorphicExpanseName === props.isomorphicExpanseName`
 ...
 *   onMounted from GET /isomorphicExpanse-anchor-spawn/<isomorphicExpanseName>), the inert "Waiting…" stub is replaced
 *   by a Pewter-styled "Spawn + Anchor this domain" button. handleSpawnAnchor spawns via the
 *   EXISTING triggerSpawnIsomorphicExpanseSession (the bridge's claimAnchorIfUnclaimed auto-anchors the page
 *   anchor), then a readiness poll (mirror IsomorphicExpanseHomeLanding PPOL · 250ms/3000ms) focuses the new
 ...
 *   the per-callsite page spawn buttons (IsomorphicExpanseHomeLanding · PewterLanding), making the menu
...
// the anchor-spawn route live in a NEVER-COPIED scsBridge model so the isomorphicExpanse:page token
// rewrite cannot break them in mints (s.isomorphicExpanseName → s.{domain}Name was the BO-1 kill).
...
// PAOLR · the page's Anchor for this isomorphicExpanseName. Keys on isAnchor (authoritative
// page-bound session), NOT "any alive isomorphicExpanse match" — mirror CadmiumLanding / STSC.
const anchor = computed<ScsBridgeSessionEntry | undefined>(() =>
  resolveS8Anchor(sessionsList.value, props.isomorphicExpanseName),
);
...
// GET /isomorphicExpanse-anchor-spawn/<isomorphicExpanseName> (server resolves the per-Suite-8 Cascade.json ·
// DEFAULT 'prompt' on any failure · mirrors IsomorphicExpanseHomeLanding AD). 'prompt' = the menu surfaces a
...
// sidesteps the server anti-flood guard that a fresh spawn would trip). Mirrors IsomorphicExpanseHomeLanding /
...
// plan selector · mirrors IsomorphicExpanseHomeLanding PPOL-WUD).
...
// /isomorphicExpanse-anchor-spawn fetch can resolve BEFORE the offline prior hydrates → !anchor.value momentarily
// true → a spurious spawn that then steals the offline original via the fallback). This guard mirrors
// CadmiumLanding / IsomorphicExpanseHomeLanding PAOLR: settle over a window, then branch ONCE on the SETTLED
...
// component; no page wires its own floor). GET /isomorphicExpanse-menu/<isomorphicExpanseName> on mount → floorStage.
...
onMounted(() => {
  void fetch(s8MenuPath(props.isomorphicExpanseName))
...
const persistStageIndex = (idx: number): void => {
  void fetch(S8_MENU_STAGE_SET_PATH, {
    ...
    body: JSON.stringify({ designation: props.isomorphicExpanseName, currentStageIndex: idx }),
  }).catch(() => {
...
// /isomorphicExpanse-anchor-spawn/<isomorphicExpanseName> · DEFAULT 'prompt' on any failure · fire-and-forget · the
...
onMounted(() => {
  ...
  void fetch(s8AnchorSpawnPath(props.isomorphicExpanseName))
    ...
    .then((j: { anchorSpawn?: string }) => {
      ...
      // spawning over it (mirror CadmiumLanding / IsomorphicExpanseHomeLanding PAOLR three-branch).
...
// CadmiumLanding / IsomorphicExpanseHomeLanding PAOLR three-branch under a one-shot settle window: poll
...
function autoDecideAnchor(): void {
  ...
  autoDecidePoll = setInterval(() => {
    ...
    if (anchorAlive.value) {
      ...
      resolveOnce(() => {
        console.log('[ShatteriteMenu P1.1] AD-Auto · anchor ALIVE · no spawn · isomorphicExpanseName=', props.isomorphicExpanseName);
      });
      ...
      resolveOnce(() => {
        ...
        if (reengageTombstoned.value) {
          console.log('[ShatteriteMenu C472] anchor OFFLINE but TOMBSTONED · no auto re-engage · isomorphicExpanseName=', props.isomorphicExpanseName);
          return;
        }
        ...
        if (anchorSpawnMode.value !== 'auto') {
          console.log('[ShatteriteMenu C488] anchor OFFLINE · Auto-Spawn OFF · the button stands · isomorphicExpanseName=', props.isomorphicExpanseName);
          return;
        }
          ...
          void fetch(s8AnchorSpawnPath(props.isomorphicExpanseName))
            ...
            .then((j: { anchorSpawn?: string }) => {
              if (j.anchorSpawn === 'auto' && !spawning.value && !anchor.value) {
                console.log('[ShatteriteMenu P1.1] AD-Auto · NO anchor after settle · server-confirmed · spawn · isomorphicExpanseName=', props.isomorphicExpanseName);
                void handleSpawnAnchor();
              } else {
                console.log('[ShatteriteMenu C471] spawn double-check REFUSED · server mode=', j.anchorSpawn, '· isomorphicExpanseName=', props.isomorphicExpanseName);
              }
            })
            .catch(() => { /* unreachable server — never spawn blind */ });
        } else {
          console.log('[ShatteriteMenu C470] NO anchor after settle · auto-spawn OFF · the button stands · isomorphicExpanseName=', props.isomorphicExpanseName);
        }
      });
    }
  }, AD_STEP_MS);
...
async function toggleAutoMode(): Promise<void> {
  ...
  try {
    const res = await fetch(s8AnchorSpawnPath(props.isomorphicExpanseName), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const reengageTombstoneKey = (): string => `scsReengageTombstone:${props.isomorphicExpanseName}`;
...
watch(() => props.isomorphicExpanseName, (name, prev) => {
  if (name && !prev) {
...
watch([showSpawnOption, showReengageOption, anchorStatus], ([spawnOpt, reengOpt, status]) => {
  ...
  // (a cross-window set otherwise leaves this window's render stale until tab-in).
    ...
    '· spawning=', spawning.value, '· isomorphicExpanseName='.replace('isomorphicExpanseName','designation'), props.isomorphicExpanseName);
});
...
watch(settledAnchorStatus, (nowStatus, prevStatus) => {
  ...
  if (prevStatus === 'launched' && nowStatus === 'offline') {
    ...
    console.log('[ShatteriteMenu C472] anchor USER-CLOSED · tombstone SET · Auto-Spawn PRESERVED · isomorphicExpanseName=', props.isomorphicExpanseName);
  }
});

// SOE · spawn + anchor THIS domain from the menu (parallel to handleOption). Spawns via the EXISTING
// triggerSpawnIsomorphicExpanseSession (the bridge sets entry.isomorphicExpanseName BEFORE spawn → claimAnchorIfUnclaimed
// auto-anchors the page anchor), then a readiness poll (250ms/3000ms · mirror IsomorphicExpanseHomeLanding PPOL
// / CadmiumLanding PAOLRP) watches sessionsList for the launched anchor. On found → focus + clear.
// Fallback (a session for this isomorphicExpanseName appeared but did NOT auto-anchor): await triggerSetAnchor.
...
async function handleSpawnAnchor(): Promise<void> {
  ...
  spawning.value = true;
  console.log('[ShatteriteMenu SOE] spawn + anchor · isomorphicExpanseName=', props.isomorphicExpanseName);

  // Spawn — the bridge claimAnchorIfUnclaimed auto-anchors the new session to this page.
  // C373 · triggerSpawnS8Session (rename-proof alias) — survives the isomorphicExpanse:page domain-token rewrite.
  ctrl.triggerSpawnS8Session(props.isomorphicExpanseName);
  ...
  spawnPoll = setInterval(() => {
    void (async () => {
      ...
      const live = sessions.find(
        (s) => s.isomorphicExpanseName === props.isomorphicExpanseName && s.isAnchor === true && s.status === 'launched',
      );
      if (live) {
      ...
      // auto-stamp. If a same-isomorphicExpanseName anchor ALREADY exists (any status — e.g. an offline prior
      ...
      if (elapsedMs >= SOE_MAX_MS) {
        if (spawnPoll) { clearInterval(spawnPoll); spawnPoll = null; }
        const existingAnchor = sessions.find((s) => s.isomorphicExpanseName === props.isomorphicExpanseName && s.isAnchor === true);
        const unanchored = sessions.find((s) => s.isomorphicExpanseName === props.isomorphicExpanseName && !s.isAnchor);
        if (existingAnchor) {
          ...
          console.warn('[ShatteriteMenu SOE] spawn settled with no session for isomorphicExpanseName=', props.isomorphicExpanseName);
        }
        spawning.value = false;
      }
    })();
...
// poll / IsomorphicExpanseHomeLanding PPOL) watches sessionsList for the anchor flipping to 'launched'. On found
...
async function handleReengageAnchor(): Promise<void> {
  ...
  console.log('[ShatteriteMenu P1] re-engage offline anchor · isomorphicExpanseName=', props.isomorphicExpanseName, '· anchorId=', target.id);
  ...
  reengagePoll = setInterval(() => {
    ...
    if (elapsedMs >= REENGAGE_MAX_MS) {
      if (reengagePoll) { clearInterval(reengagePoll); reengagePoll = null; }
      console.warn('[ShatteriteMenu P1] re-engage did not settle alive for isomorphicExpanseName=', props.isomorphicExpanseName);
      isReengaging.value = false;
    }
  }, REENGAGE_STEP_MS);
}
...
// (1) GET the SORD-wrapped Skill/Strategy envelope from /isomorphicExpanse-skill-prime, now wrapped
...
async function primeSend(
  ...
  const url = `/isomorphicExpanse-skill-prime/${encodeURIComponent(props.isomorphicExpanseName)}?ref=${encodeURIComponent(ref)}&kind=${primeKind}`;
  ...
  try {
    ...
    const res = await ctrl.triggerSendMessage(targetId, j.envelope);
    return res?.ok ?? false;
  } catch (err) {
...
async function handleOption(option: MenuOption): Promise<void> {
  ...
  const target = anchor.value;
  if (!ctrl || !target) {
    console.warn('[ShatteriteMenu SMSP] no alive Anchor for option · isomorphicExpanseName=', props.isomorphicExpanseName);
  ...
  try {
    if (option.kind === 'scs') {
      ...
      // server (/isomorphicExpanse-skill-prime), then relay the envelope to the live Anchor (triggerSendMessage)
      ...
      if (!ref) {
        ...
        ok = await primeSend(ctrl, target.id, ref, option.primeKind ?? 'skill', option.label);
        if (!ok) {
...
async function handleSubmit(option: MenuOption, i: number): Promise<void> {
  ...
  if (typeof pairDirective === 'string' && pairDirective.length > 0) {
    ...
    const target = anchor.value;
    if (!ctrl || !target) {
      console.warn('[ShatteriteMenu TRP] no alive Anchor for pairing submit · isomorphicExpanseName=', props.isomorphicExpanseName);
      emit('option-selected', { label: option.label, kind: option.kind, ok: false });
      return;
    }
  ...
  const target = anchor.value;
  if (!ctrl || !target) {
    console.warn('[ShatteriteMenu MOIS] no alive Anchor for submit · isomorphicExpanseName=', props.isomorphicExpanseName);
    emit('option-selected', { label: option.label, kind: option.kind, ok: false });
    return;
  }
...
<template>
  ...
  >
    ...
    <div v-if="showSpawnOption" class="menu-stub menu-stub-spawn">
      <span class="menu-stub-text">No Anchor yet for {{ isomorphicExpanseName }} — engage to begin.</span>
      <button
        type="button"
    ...
    <!-- P1 · ORPHAN-ANCHOR RE-ENGAGE · an anchor exists but is offline (status !== 'launched').
         ...
         render (no extra wiring). Mirrors IsomorphicExpanseHomeLanding / CadmiumLanding PPOL branch (2). -->
    <div v-else-if="showReengageOption" class="menu-stub menu-stub-spawn">
      <span class="menu-stub-text">
        Anchor for {{ isomorphicExpanseName }} is offline — re-engage to bring it back online.
      </span>
      <button
        type="button"
        class="menu-option-btn menu-spawn-btn"
