...
/**
 * IsomorphicExpanseCascadeDocs — the D-O Muxified Read, as a generic Suite 8 page surface.
 *
 ...
 * (d.client.d.suiteCascade.k.cascades) — the exact precedent IsomorphicExpanseLanding.vue:311
 * establishes — and the routes are the generic /isomorphicExpanse-doc-tiers + /isomorphicExpanse-doc-save pair.
 *
 * Write authority split by nature:
 *   - The plan pane (Diamond · prunable) is PAGE-EDITABLE — a textarea toggle saves via the
 *     path-guarded /isomorphicExpanse-doc-save endpoint (writes DIAMOND-*.md only · DIAMOND-only · 403 ONYX).
 ...
 * WITHOUT loading their content (a lightweight directory listing via /isomorphicExpanse-doc-tiers),
 ...
 *
 * Render seat: mounted by IsomorphicExpanseHomeLanding (the ZONE 1 WTO Triptych RI-widget seat · replaces
 * the prop-fed SuiteCascadeDiamondOnyxPane). Reads the shared Tier-2 suiteCascade member.
 *
 * Pattern precedent: IsomorphicExpanseLanding.vue (Tier-2 cascades subscription) ·
...
// W1 (C758) · REWRITE-PROOF ROUTES — the /isomorphicExpanse-* literals live ONLY in the never-copied
// scsBridge s8Routes model; the mint's isomorphicExpanse-token rewrite cannot break these fetch paths.
...
// per-designation in localStorage under a namespaced key (`isomorphicExpanse-cascade-docs:<designation>` ·
...
const sectionExpanded = ref<boolean>(true);
const sectionStorageKey = computed<string>(() => `isomorphicExpanse-cascade-docs:${props.designation}`);
...
// fetch does not. GET /isomorphicExpanse-cascade/<designation> reads Cascade.json + the manifest's
...
async function hydrateCascadeBootFloor(): Promise<void> {
  try {
    ...
    console.log('[IsomorphicExpanseCascadeDocs] on-boot self-query floored · files=', entries.length);
  } catch {
...
onMounted(() => {
  ...
  muxium = createClientMuxiumInstance<ClientMuxiumDeck>(
    ...
    {
      title: 'IsomorphicExpanseCascadeDocs',
      logging: false,
      storeDialog: true,
    },
  );

  // Tier-2 subscription — the shared suiteCascade member's cascades Record (IsomorphicExpanseLanding.vue:311
  // precedent). Reads THIS designation's key.
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'isomorphicExpanseCascadeDocsSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
  ...
  mountHydrationPlanner = muxium.plan<ClientMuxiumDeck>(
    'isomorphicExpanseCascadeDocsConnectionGatedMountHydrationSingleStage',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
