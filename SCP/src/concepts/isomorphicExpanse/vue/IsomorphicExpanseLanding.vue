 ...
 * the isomorphicExpanse CLIENT face per-page; createIsomorphicExpanseClientConcept() returns the SCBM
 * muxifyConcepts node — isomorphicExpanse (Tier 1) carrying the ONE shared suiteCascade member
 * (Tier 2 · d.suiteCascade). The MPRF registration Principle seeds the
 * `isomorphicExpanses` roster at boot.
 *
 ...
 *   Home          -> the Suite 8 roster (isomorphicExpanses · Tier 1) + the docked-cascade
 *                    context (cascades · Tier 2 · the SCBM payoff)
 *   Component     -> CPLD live render — ODSS spawn (A-4) + PFGD list (A-5), keyed
 *                    by the isomorphicExpanses plurality
 *   Documentation -> the IsomorphicExpanse registration/assignment reference (static markdown)
 *
 * MIGRATION (deferred A-1 item): the Home roster reads `isomorphicExpanses` (the SPSR Record)
 * via DECK K, NOT the legacy `designations[]` array. The legacy array + its 3
 * sync'd qualities remain on the concept (IsomorphicExpansePage.vue still reads them) — this
 * Landing's roster is the canonical `isomorphicExpanses` surface.
 *
 * Citation: SuiteCascadeLanding.vue (HCD triad v-if switch · DIRECT bearing · B-6).
 * Citation: IsomorphicExpanseOnDemand.vue / IsomorphicExpanseSessionList.vue (IUPA + GPIM controller idiom).
 * Citation: MASTER-DIAMOND-ISOMORPHICEXPANSE-CONCEPT-ASPIRANT.md §3 (HCD) + §5 (Diametric Conference).
 */
...
import type { IsomorphicExpanseEntry, IsomorphicExpanseSubPage } from '../isomorphicExpanse.type';
...
// A-1 SCBM · IUPA · createIsomorphicExpanseClientConcept() returns the muxifyConcepts node —
// isomorphicExpanse (Tier 1) carrying the ONE shared suiteCascade member (Tier 2).
...
import { ISOMORPHICEXPANSE_SUB_PAGE_OPTIONS } from '../isomorphicExpanse.subPageRegistry';
...
import IsomorphicExpanseSubPageNav from './components/IsomorphicExpanseSubPageNav.vue';
import IsomorphicExpanseComponentSubPage from './components/IsomorphicExpanseComponentSubPage.vue';
import IsomorphicExpanseDocumentationSubPage from './components/IsomorphicExpanseDocumentationSubPage.vue';
...
import IsomorphicExpanseBiplane from './components/IsomorphicExpanseBiplane.vue';

// --- Reactive refs mirrored from isomorphicExpanse state via the stage-planner subscription.
// isomorphicExpanses + activeSubPage = Tier 1 (own); cascades + activeCascadeDirectory = Tier 2
// (the shared suiteCascade member · the SCBM payoff).
const isomorphicExpanses = ref<Record<string, IsomorphicExpanseEntry>>({});
const activeSubPage = ref<IsomorphicExpanseSubPage>('home');
...
const subPageOptions = ISOMORPHICEXPANSE_SUB_PAGE_OPTIONS;

// ============================================
// MD-3 · THE NAME-FIRST DEMOMETRIC MINT — the Create UI + the sovereignty-pure roster
// ============================================
// SOVEREIGNTY: the roster reads THIS SCP's OWN Cascades/8_SUITES via GET /isomorphicExpanse/local-roster
// (SCP-local · no bridge SEAP dependency · no static-seed GAP). A minted Suite 8 appears the
// instant its dir exists — refreshLocalRoster re-fetches + dispatches isomorphicExpanseRegisterIsomorphicExpanse per
// entry into the SPSR Record (the existing subscription mirrors it into isomorphicExpanses.value).

// MD-5 · the local-roster entry now carries the SNIPPET (the first meaningful Instance.md line ·
// the extended /isomorphicExpanse/local-roster · the MD-4 firstLine helper idiom).
...
// MD-5 · snippet map keyed by Suite 8 name — held here (NOT on the IsomorphicExpanseEntry state · the
...
// D-EF-3 · THE FORGE PREDICATE map — keyed by Suite 8 name (held OFF the IsomorphicExpanseEntry state · same
...
// query param on mount; once the roster load resolves that entry into isomorphicExpanses, auto-expand it (the
...
// isomorphicExpanseRegisterIsomorphicExpanse per dir entry (idempotent · the reducer overwrites by name). Missing
// description/color (the local listing carries only name+path) → deterministic placeholders.
async function refreshLocalRoster() {
  if (!muxium) return;
  try {
    const r = await fetch('/s8/local-roster'); // C370-B · rename-proof prefix (aliased to /isomorphicExpanse)
    if (!r.ok) return;
    ...
    for (const e of entries) {
      ...
      const entry: IsomorphicExpanseEntry = {
        name: e.name,
      ...
      (muxium as Muxium<ClientMuxiumDeck>).dispatch(
        (muxium as Muxium<ClientMuxiumDeck>).deck.d.client.d.isomorphicExpanse.e.isomorphicExpanseRegisterIsomorphicExpanse({
          name: e.name,
          entry,
        }),
      );
...
async function submitCreate() {
  ...
  createError.value = '';
  try {
    const r = await fetch('/s8/create', { // C370-B · rename-proof prefix (aliased to /isomorphicExpanse)
      method: 'POST',
...
// --- Home derivation · the Suite 8 roster (isomorphicExpanses Record · Tier 1).
const isomorphicExpanseEntries = computed<IsomorphicExpanseEntry[]>(() => Object.values(isomorphicExpanses.value));

// MD-5 · resolve the expanded selection to its entry (null → the compact grid).
const expandedEntry = computed<IsomorphicExpanseEntry | null>(() =>
  expandedName.value ? isomorphicExpanses.value[expandedName.value] ?? null : null,
);
...
function selectSubPage(subPage: IsomorphicExpanseSubPage) {
  if (!muxium) return;
  muxium.dispatch(
    (muxium as Muxium<ClientMuxiumDeck>).deck.d.client.d.isomorphicExpanse.e.isomorphicExpanseSetActiveSubPage({
...
onMounted(() => {
  ...
  // isomorphicExpanses watcher above can auto-expand that entry the instant refreshLocalRoster resolves it.
  try {
  ...
  // IUPA · landing supplies isomorphicExpanse (SCBM node) as the muxonomic page concept.
  muxium = createClientMuxiumInstance<ClientMuxiumDeck>(
    ...
    {
      title: 'IsomorphicExpanseLanding',
      logging: true,
      storeDialog: true,
    },
  );
  ...
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'isomorphicExpanseLandingSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            // Tier 1 — own roster Record + local SubPage selector.
            isomorphicExpanses.value = d.client.d.isomorphicExpanse.k.isomorphicExpanses.select();
            activeSubPage.value = d.client.d.isomorphicExpanse.k.activeSubPage.select();
          ...
          {
            selectors: [
              d__.client.d.isomorphicExpanse.k.isomorphicExpanses,
              d__.client.d.isomorphicExpanse.k.activeSubPage,
  ...
  // dispatched isomorphicExpanseRegisterIsomorphicExpanse actions land on the bound base.
...
<template>
  <div class="isomorphicExpanse-landing">
    ...
    <main class="landing-content">
      <IsomorphicExpanseSubPageNav
        :options="subPageOptions"
        :active-sub-page="activeSubPage"
        @sub-page-selected="selectSubPage"
      />

      <!-- HOME · the Suite 8 roster (isomorphicExpanses · Tier 1) + docked-cascade context (Tier 2) -->
      <section v-if="activeSubPage === 'home'" class="home-subpage">
        ...
        <!-- The Suite 8 roster · isomorphicExpanses Record (Tier 1 · the migrated A-1 surface) -->
        <section class="roster-panel hifi-pane-amethyst">
          <h2 class="hifi-heading">Registered Suite 8 Roster</h2>

          <p v-if="isomorphicExpanseEntries.length === 0" class="roster-empty-note">
          ...
          <!-- MD-5 · THE ROSTER SURFACE SWAP — one IsomorphicExpanseCard (compact) per entry; card click -->
          ...
          <div v-else class="roster-grid">
            <IsomorphicExpanseCard
              v-for="entry in isomorphicExpanseEntries"
              :key="entry.name"
              :entry="entry"
            ...
            <div class="roster-create-cell">
              ...
              <div v-else class="create-form">
                <label class="create-field-label" for="isomorphicExpanse-create-name">
                  New Suite 8 Name
                </label>
                <input
                  id="isomorphicExpanse-create-name"
                  v-model="createName"
                  class="create-input"
                  type="text"
      ...
      <!-- COMPONENT · CPLD live render — ODSS spawn (A-4) + PFGD list (A-5) keyed by isomorphicExpanses -->
      <IsomorphicExpanseComponentSubPage
        v-else-if="activeSubPage === 'component'"
        :isomorphicExpanses="isomorphicExpanses"
      />

      <!-- DOCUMENTATION · the IsomorphicExpanse registration/assignment reference -->
      <IsomorphicExpanseDocumentationSubPage v-else-if="activeSubPage === 'documentation'" />
    </main>
  </div>
</template>

<style scoped>
.isomorphicExpanse-landing {
  min-height: 100vh;
  padding: 2rem;
