/**
 * IsomorphicExpanse Concept State Factory (Huirth Deployment) · GTMS8C Base
 *
 * The server-side (Base) state for the isomorphicExpanse Demometer — holds menuStage as the Huirth
 ...
 * menu.json exists. The thin menu-watch dir-watch (isomorphicExpanseMenuWatch) writes real stages via the
 * Base quality (isomorphicExpanseSetMenuStageHuirthBase); JDIS unlink resets it back to EMPTY_MENU_STAGE.
 *
 ...
 */
import type { IsomorphicExpanseHuirthState } from './isomorphicExpanse.type';
import { EMPTY_MENU_STAGE } from '../../model/shatteriteMenu.model';

export function createIsomorphicExpanseHuirthState(): IsomorphicExpanseHuirthState {
  return {
    // KeyedSelector discipline · NON-OPTIONAL · the menu-watch dir-watch writes real stages via
    // the Base quality (isomorphicExpanseSetMenuStageHuirthBase); an unlink resets it to EMPTY_MENU_STAGE.
    menuStage: EMPTY_MENU_STAGE,

    // PRE-EPOCH · BSSM keyed Record (Base mirror). Always {} at boot (KeyedSelector). The N-watcher
    // dir-watch writes per-designation stages via isomorphicExpanseSetDesignationMenuStageHuirthBase; the SMRP
